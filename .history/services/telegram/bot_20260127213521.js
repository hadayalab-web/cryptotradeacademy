...
async function sendMessageToAsset(text, asset = 'BTC', langCode = null, options = {}) {
  ...
  if (asset === 'MINIMAL' && langCode) {
    ...
    if (!chatId) {
      ...
      console.warn(`⚠️ Language-specific channel ID not found for MINIMAL/${langCodeUpper}, and default MINIMAL channel is missing`);
    }
  } else {
  ...
  if (!botToken || !chatId) {
    ...
    const errorMsg = `Telegram credentials missing for ${assetLabel}. Bot Token: ${!!botToken}, Chat ID: ${!!chatId}`;
    console.error(`❌ ${errorMsg}`);
...
async function sendMessageToChannel(text, series = 'BTC', marketCode = 'EN', options = {}) {
  ...
  if (!botToken || !chatId) {
    const errorMsg = `Telegram credentials missing for ${series}/${marketCode}. Bot Token: ${!!botToken}, Chat ID: ${!!chatId} (env: ${envVarName})`;
    console.error(`❌ ${errorMsg}`);
    throw new Error(errorMsg);
  }
