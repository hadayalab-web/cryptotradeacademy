...
/**
 * 初速ターボモード: QUOTE_REPOST_TURBO_MODE または X_TURBO_MODE が true のとき上限を引き上げ
 */
function isTurboMode() {
  return process.env.QUOTE_REPOST_TURBO_MODE === "true" || process.env.X_TURBO_MODE === "true";
}
 ...
 * 初速ターボ: 400/日（X API余裕内で初速を最大化）
 * 環境変数で変更可能: X_MAX_DAILY_POSTS（デフォルト: 通常250 / ターボ400）
 */
function checkDailyPostLimit(currentPostCount, maxPosts = null) {
  const defaultNormal = 250;
  const defaultTurbo = 400;
  const defaultMaxPosts = process.env.X_MAX_DAILY_POSTS
    ? parseInt(process.env.X_MAX_DAILY_POSTS, 10)
    : isTurboMode()
      ? defaultTurbo
      : defaultNormal;
 ...
 * X APIレート制限: Per User 100/15min（= 400/時間理論上）
 * 初速ターボ: 80/時間（400/日を均等配分しつつ100/15min未満に収める）
 * 環境変数で変更可能: X_MAX_HOURLY_POSTS（デフォルト: 通常100 / ターボ80）
 * @param {number} currentHourlyPostCount - 現在の1時間あたりの投稿数
 * @param {number} maxPostsPerHour - 1時間あたりの最大投稿数
 * @returns {boolean} 投稿可能な場合 true
 */
function checkHourlyPostLimit(currentHourlyPostCount, maxPostsPerHour = null) {
  const defaultNormal = 100;
  const defaultTurbo = 80; // 400/24 ≒ 17/時を許容しつつ、ピーク時バースト用に80
  const defaultMaxPostsPerHour = process.env.X_MAX_HOURLY_POSTS
    ? parseInt(process.env.X_MAX_HOURLY_POSTS, 10)
    : isTurboMode()
      ? defaultTurbo
      : defaultNormal;
