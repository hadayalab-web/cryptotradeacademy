...
async function handleUserReplies(mainTweetId, lang, reportData = null, maxReplies = 10) {
  ...
  if (kv) {
    try {
      ...
      console.log(`[User Reply Handler] Marked tweet ${mainTweetId} as processing (duplicate prevention)`);
    } catch (err) {
  ...
  console.log(`[User Reply Handler] Scheduled ${replies.length} user replies for tweet ${mainTweetId}`);
  return results;
}
