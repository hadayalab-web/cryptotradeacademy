...
function calculateExpectedPerformance(impressions, engagementRate, hasTelegramLink, hasWhopLink) {
  ...
  const telegramOptIns = telegramClicks
    ? {
        conservative: Math.round(
          telegramClicks.conservative * CONVERSION_RATES.telegramOptInRate.conservative
        ),
        moderate: Math.round(telegramClicks.moderate * CONVERSION_RATES.telegramOptInRate.moderate),
        optimistic: Math.round(
          telegramClicks.optimistic * CONVERSION_RATES.telegramOptInRate.optimistic
        )
      }
    : null;
  ...
  const whopConversions = whopClicks
    ? {
        conservative: Math.round(
          whopClicks.conservative * CONVERSION_RATES.whopConversionRate.conservative
        ),
        moderate: Math.round(whopClicks.moderate * CONVERSION_RATES.whopConversionRate.moderate),
        optimistic: Math.round(
          whopClicks.optimistic * CONVERSION_RATES.whopConversionRate.optimistic
        )
      }
    : null;
...
async function analyzePostPerformance(dateString) {
  ...
  for (const post of posts) {
    ...
    if (influencer) {
      ...
      performance = calculateExpectedPerformance(
        impressions,
        engagementRate,
        hasTelegramLink,
        hasWhopLink
  ...
  return {
    ...
    summary: {
      ...
      totalWhopConversions:
        Object.keys(totalWhopConversions).length > 0 ? totalWhopConversions : null,
...
async function calculateDailyExpectations() {
  ...
  // Minimal Version: 2投稿/日（実際のCronスケジュール: UTC 8,20で実行、全6言語対応）
  // 注意: Grokの推奨とは異なり、実際のCronスケジュールに基づく
  ...
  const totalImpressions =
    quoteRepostImpressions + freeReportImpressions + minimalVersionImpressions;
  ...
  const telegramClicks = {
    conservative: Math.round(
      telegramLinkImpressions * CONVERSION_RATES.telegramClickRate.conservative
    ),
