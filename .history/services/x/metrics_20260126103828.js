...
async function getTweetMetrics(tweetId, includeNonPublic = false, options = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`[X Metrics] ✅ Successfully retrieved metrics for tweet ${tweetId} (attempt ${attempt + 1}/${maxRetries + 1})`);
      return metrics;
    } catch (error) {
      lastError = error;
      ...
      if (attempt < maxRetries) {
        console.warn(`[X Metrics] ⚠️ Failed to get metrics for tweet ${tweetId} (attempt ${attempt + 1}/${maxRetries + 1}): ${error.message}. Retrying in ${delay}ms...`);
        await new Promise(resolve => setTimeout(resolve, delay));
      } else {
        console.error(`[X Metrics] ❌ CRITICAL: Failed to get metrics for tweet ${tweetId} after ${maxRetries + 1} attempts: ${error.message}`);
      }
    }
  }
  ...
  const finalErrorDetails = {
    ...
    response: lastError?.response ? (typeof lastError.response === 'string' ? lastError.response.substring(0, 200) : JSON.stringify(lastError.response).substring(0, 200)) : null,
  };
  
  throw new Error(`CRITICAL: Failed to get metrics for tweet ${tweetId} after ${maxRetries + 1} attempts. Last error: ${finalErrorDetails.message}${finalErrorDetails.status ? ` (Status: ${finalErrorDetails.status})` : ''}${finalErrorDetails.response ? ` (Response: ${finalErrorDetails.response})` : ''}`);
}
