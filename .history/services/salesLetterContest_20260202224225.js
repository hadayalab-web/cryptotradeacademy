...
/**
 * Grok専用: ヘッドライン → 本文（Minimal/Regularチラ見せ）→ ハッシュタグまで一気に書かせるプロンプト
 */
function buildGrokOnlyPrompt({ lang, reportData }) {
  ...
  return `You are a direct-response copywriter for Trap Defence (crypto trading education). Write a complete X (Twitter) post in ${langName} in THREE parts. Use the persona and on-chain data below—they are mandatory.
...
1) HEADLINE
One short line that grabs attention (question, pain, or curiosity). Reference Trap Score or market if it fits. No hashtags here.

2) BODY (3–4 short paragraphs)
- Paragraph 1: Hook with persona + market (e.g. "just watching" loop, unrealized loss, Trap Score).
- Paragraph 2: Tease FREE Minimal Version—Trap Score, gut vs data, no card. What they see, why it matters, what they miss if they don't try. End with a VSL CTA: direct readers to watch the FREE VSL (e.g. "Watch the free VSL below for the full story" / "無料VSLで全体像を、下のリンクから視聴").
- Paragraph 3: Tease PAID Regular Briefing—15min Alerts + Exit Map, $99/mo, 1-day trial, risk zero. Code DEFEND50 for 50% off (once). End with a VSL CTA: direct readers to the UPGRADE VSL (e.g. "Upgrade VSL: link below" / "有料アップグレード用VSLは下のリンクで").
- Paragraph 4 (optional): Soft close. "You're not alone. Get the framework. Act now." Optionally add one line: "Free VSL + signup link below. Upgrade VSL + Whop link below."
No actual URLs in the body—we add VSL and Whop links when posting. Tone: ${TONE}. Every sentence ends with a period (or 。 in Japanese).

3) HASHTAGS
End with 2–4 hashtags on a new line. Include #BTC and #TrapDefence. Add 1–2 more if they fit (e.g. #Crypto #Trading).

Output the full post: headline, then body (with VSL guidance in paragraphs 2–3), then hashtags. No other labels or section headers—just the headline line, blank line, body paragraphs, blank line, hashtag line.`;
}

/**
 * Grok-4-1-fast-reasoning のみでセールスレター生成（ヘッドライン → 本文 → ハッシュタグ）
 * @param {Object} options - { lang, reportData }
 * @returns {Promise<{ prompt: string, text: string|null }>}
 */
