...
- 3–5 short paragraphs. Hook with persona + market, then tease Minimal, then tease Regular, then soft close.
- Tone: ${TONE}
...
async function generateWithGemini({ lang, prompt }) {
  ...
  try {
    const text = await callGeminiForQuoteRepost(prompt, 800);
    return text || null;
  } catch (err) {
