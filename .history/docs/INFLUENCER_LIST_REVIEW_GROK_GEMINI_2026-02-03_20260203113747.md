...
**整合メモ**: **引用リポストの実運用で参照するリストは KV の 300 件**。824/840 はファイル確定数（2周目・data/influencers）の目標/達成数。KV リスト数は `config/influencerStrategy.js` の STOCK_COUNT_BY_LANG 合計（300）に合わせる。
...
- **824/840**: ファイル確定数（2周目・`data/influencers/*.json`）およびドキュメント上の目標/達成数。KV とは別。
