...
- **引用リポストは無料＋有料を1投稿に統合**  
  X投稿は **引用リポストのみ**。**300のインフルエンサーアカウント**（KVストック）に対して、**1投稿内で無料版（Minimal）と有料版（Regular）の両導線**を出す。バリアントは A〜E をランダム選択（ドローダウン時は D を50%に加重）。導線の交互ローテーションは廃止（`config/quoteRepostTemplatesIntegrated.js`）。  
  詳細: `docs/QUOTE_REPOST_MINIMAL_REGULAR_ROTATION.md`、`docs/STRATEGY_TUNING_PERSONA_2026-02.md`
