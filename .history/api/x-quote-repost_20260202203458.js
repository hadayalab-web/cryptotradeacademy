...
async function generateQuoteRepostTextWithGrok(
  ...
  try {
    ...
    // Whopはリンクだけ（X投稿ポリシー）。GrokにはWhop URLを渡さず、ここで必ず末尾に付与する
    const regularBriefingWhopUrl = getRegularWhopLinkOnly(lang);
    const minimalCheckoutUrl =
      getMinimalVersionCheckoutUrl(normalizeLang(lang), {
        influencerUsername: influencerTweet.username,
        utm_content: `influencer_${influencerTweet.username}`
      }) || "";

    // 最適化戦略をプロンプトに反映（generateQuoteRepostTextに渡す）
    let quoteText = await generateQuoteRepostText(
      lang,
      influencerTweet,
      reportData,
      deepLink,
    ...
    // Whopをリンクだけで末尾に付与（Grokの出力に任せず、コードで保証。既にGrokが入れたWhop URLは削除して置き換え）
    const whopLinksOnly = [minimalCheckoutUrl, regularBriefingWhopUrl].filter(Boolean).join(" ");
    if (whopLinksOnly) {
      quoteText = quoteText
        .replace(/https?:\/\/whop\.com\/[^\s]*/g, "")
        .replace(/\s+/g, " ")
        .trim();
      quoteText = `${quoteText} ${whopLinksOnly}`.trim();
    }
    return quoteText;
  } catch (error) {
    ...
    for (const influencer of influencers.slice(0, maxInfluencers)) {
      ...
      try {
        ...
        if (useMinimalRegularTemplates) {
          // 無料版（Minimal）＋有料版（Regular）導線を1投稿に統合（両方のCTAを出す）
          ...
          console.log(
            `[Quote Repost] 📌 Integrated Minimal+Regular: @${influencer.username} (variant ${variant}${drawdown ? ", drawdown weighted" : ""}) [runId: ${langRunId}]`
          );
        } else if (isDryRun) {
