      ...
      while (retries > 0 && !success) {
        try {
          ...
          const impressions = metrics.nonPublicMetrics?.impression_count ?? 
                              ...
                              0;
          const clicks = metrics.nonPublicMetrics?.url_link_clicks ?? 
          ...
          if (engagementMetrics.impressions === 0) {
            ...
            if (minutesSincePost < 10) {
              console.log(`[X Engagement Metrics] ⚠️ Impressions is 0 for tweet ${post.tweetId} (posted ${minutesSincePost.toFixed(1)} minutes ago - normal, will retry later)`);
            } else {
              console.warn(`[X Engagement Metrics] ⚠️ Impressions is still 0 for tweet ${post.tweetId} after ${minutesSincePost.toFixed(1)} minutes - may need investigation`);
            }
          }
          ...
          if (recordSuccess) {
            success = true;
            successCount++;
            console.log(`[X Engagement Metrics] ✅ Successfully updated metrics for tweet ${post.tweetId} (impressions: ${engagementMetrics.impressions})`);
          } else {
          ...
          retries--;
          if (retries > 0) {
            console.warn(`[X Engagement Metrics] ⚠️ Retrying metrics update for tweet ${post.tweetId} (${retries} retries left):`, error.message);
            // 指数バックオフ（2秒、4秒、8秒）
            await new Promise(resolve => setTimeout(resolve, Math.pow(2, 3 - retries) * 1000));
          } else {
            console.error(`[X Engagement Metrics] ❌ CRITICAL: Failed to update metrics for tweet ${post.tweetId} after 3 retries:`, error.message);
            failureCount++;
