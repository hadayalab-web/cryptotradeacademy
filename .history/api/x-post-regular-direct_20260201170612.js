...
async function fetchLatestMarketData() {
  try {
    ...
    try {
      ...
      console.warn(
        "[X Post Regular Direct] Failed to fetch trap score, using default:",
        error.message
      );
    }
  ...
  try {
    ...
    if (kv) {
      ...
      if (alreadyPosted === true || alreadyPosted === "true") {
        console.log(
          `[X Post Regular Direct] ⏰ Regular direct post already sent today (${dateString}), skipping`
        );
    ...
    for (let i = 0; i < targetLangs.length; i++) {
      ...
      try {
        ...
        const tweetTemplate =
          DIRECT_REGULAR_TEMPLATES[normalizedLang] || DIRECT_REGULAR_TEMPLATES.en;
