    ...
    return (async function tryBody() {
      ...
      if (shouldCallGPT) {
        ...
        try {
          ...
          gptCryptoQuantAnalysis = await analyzeCryptoQuantData(
            cryptoQuantData,
            marketContext,
            LANG
          );
      ...
      // 7-A-MINIMAL-FIRST: 無料版を先に実行（PT/AR/JA定時配信の根本対策。300秒タイムアウトで後半が欠落する問題を解消）
      ...
      const targetLangsForMinimalCheck = getTargetLanguagesForMinimal();
      const hasAnyMinimalChatId = targetLangsForMinimalCheck.some(
      ...
      if (
        ...
        else if (cqDeep && cqDeep.trapScore != null) minimalTrapScore = cqDeep.trapScore;
        else if (trap && trap.isTrap)
          minimalTrapScore =
            trap.confidence === "HIGH" ? 80 : trap.confidence === "MEDIUM" ? 50 : 30;
        else if (trap && !trap.isTrap) minimalTrapScore = 15;

        let whaleRatioValue = null;
        if (cqDeep?.whaleFlows?.whaleRatio != null) whaleRatioValue = cqDeep.whaleFlows.whaleRatio;
        else if (highResCQData?.whaleRatio != null) whaleRatioValue = highResCQData.whaleRatio;
        else if (cqDeep?.whaleRatio != null) whaleRatioValue = cqDeep.whaleRatio;
        ...
        const minimalMarketData = { mpi, priceUsd, change24h, score: snapshot.market_score };
            ...
            if (ENABLE_TELEGRAM) {
              ...
              if (minimalChatId) {
                ...
                await sendMessageToAsset(
                  minimalText,
                  "MINIMAL",
                  targetLang.toUpperCase().replace("-", "_"),
                  { reply_markup: socialProofButton }
                );
                console.log(`[Free Version] Sent successfully to ${targetLang}`);
              }
            }
          } catch (langError) {
            console.error(`[MINIMAL] ❌ Error for ${targetLang}:`, langError.message);
          }
        }
        ...
        if (ENABLE_MINIMAL_VERSION && shouldSend && force) {
          try {
            ...
            if (typeof postMinimalVersionToX === "function") {
              postMinimalVersionToX(targetLangsForMinimal, {
                trapScore: minimalTrapScore,
                priceUsd,
                change24h,
                trapData: {
          ...
          try {
            ...
            try {
              ...
              if (showContent) {
                ...
                console.log(
                  `[Gemini Show Producer] Show production returned null for ${targetLang}`
                );
              }
            } catch (error) {
              console.warn(
            ...
            console.error(
              `[REGULAR] ❌ Error processing language ${targetLang}:`,
              langError.message
            );
      ...
      if (ENABLE_EVENT_DRIVEN && triggerType === "STANDBY_BREAK") {
        ...
        try {
          ...
          console.warn(
            "[Dr. Grok] Error providing psychological support for WATCH:",
            error.message
          );
        }
