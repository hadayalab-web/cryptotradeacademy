  ...
  try {
    ...
    for (const influencer of influencers.slice(0, maxInfluencers)) {
      ...
      try {
        ...
        // 生命線: 引用リポストはテンプレ＋ローテーションのみ。Grokは使わない（精度問題のため廃止）
        ...
        if (useMinimalRegularTemplates) {
          // 無料版（Minimal）＋有料版（Regular）導線を1投稿に統合。テンプレローテーションで安定品質（Grok廃止）
          const drawdown = isDrawdown(reportData);
          const forcedVariant = process.env.QUOTE_REPOST_VARIANT;
          const variant =
            forcedVariant && ["A", "B", "C", "D", "E", "F", "G", "H"].includes(forcedVariant)
              ? forcedVariant
              : drawdown
                ? pickVariantWithWeight(getDrawdownVariantWeight())
                : INTEGRATED_VARIANTS[Math.floor(Math.random() * INTEGRATED_VARIANTS.length)];

          if (variant === "H") {
            // バリアントH（簡易）: テーマ渡し → セールスレター本文1回生成 → 下部CTA
            const { generateSalesLetterFromTheme } = require("../services/gemini/quoteRepostCopy");
            const theme =
              process.env.QUOTE_REPOST_THEME ||
              (drawdown
                ? "drawdown"
                : ["trap_score", "whale_accumulation", "fear_greed"][
                    Math.floor(Math.random() * 3)
                  ]);
            const apiTimeoutMs = deadlineMs
              ? Math.min(14000, Math.max(2000, deadlineMs - Date.now() - 6000))
              : 14000;
            let bodyText = null;
            if (!isDryRun) {
              try {
                bodyText = await withTimeout(
                  generateSalesLetterFromTheme({ lang, theme, reportData }),
                  apiTimeoutMs
                );
              } catch (err) {
                console.warn(
                  `[Quote Repost] Gemini variant H theme fallback (${err.message}): @${influencer.username} [runId: ${langRunId}]`
                );
              }
            }
            const bodyFallback =
              (lang === "ja" ? CORE_PHRASES.state.ja : null) || CORE_PHRASES.state.en;
            const ctaBlock = getSimpleCtaWithLinks(lang, {
              influencerUsername: influencer.username
            });
            quoteText = `${bodyText || bodyFallback}\n\n${ctaBlock}`;
            funnelTypeUsed = "integrated";
            console.log(
              `[Quote Repost] 📌 Template (variant H, theme=${theme}): @${influencer.username} [runId: ${langRunId}]`
            );
          } else if (variant === "G") {
            // バリアントG: ヘッドライン → Gemini(CQ＋無様なペルソナ) → チラ見せ＋リンク説明 → GPT-5-mini(CQ＋反論) → CTA
            ...
            const apiTimeoutMs = deadlineMs
              ? Math.min(12000, Math.max(2000, deadlineMs - Date.now() - 8000))
              : 12000;
            let hookText = null;
            let objectionText = null;
            if (!isDryRun) {
              try {
                hookText = await withTimeout(
                  generatePsychologyHook({ lang, reportData }),
                  apiTimeoutMs
                );
              } catch (err) {
                console.warn(
                  `[Quote Repost] Gemini variant G hook fallback (${err.message}): @${influencer.username} [runId: ${langRunId}]`
                );
              }
              try {
                objectionText = await withTimeout(
                  generateQuoteRepostObjection({ lang, reportData }),
                  apiTimeoutMs
                );
              } catch (err) {
                console.warn(
                  `[Quote Repost] GPT-5-mini objection fallback (${err.message}): @${influencer.username} [runId: ${langRunId}]`
                );
              }
            }
            const hookFallback =
            ...
            quoteText = [
              parts.headline,
              hookText || hookFallback,
              parts.minimalBlock,
              parts.regularBlock,
              objectionText || parts.objectionFixed,
              parts.promoBlock
            ].join("\n\n");
            funnelTypeUsed = "integrated";
            console.log(
              `[Quote Repost] 📌 Template (variant G, Gemini hook + GPT-5-mini objection): @${influencer.username} [runId: ${langRunId}]`
            );
          } else {
            quoteText = getIntegratedMinimalRegularQuoteTemplate(lang, {
              variant,
              influencerUsername: influencer.username
            });
            funnelTypeUsed = "integrated";
            ...
            console.log(
              `[Quote Repost] 📌 Template (variant ${variant}): @${influencer.username} [runId: ${langRunId}]`
            );
          }
        } else if (isDryRun) {
          ...
          // Grok廃止: QUOTE_REPOST_USE_MINIMAL_REGULAR_TEMPLATES=false でもテンプレで統一（生命線の精度を保証）
          const drawdown = isDrawdown(reportData);
          ...
          console.log(
            `[Quote Repost] 📌 Template fallback (variant ${variant}): @${influencer.username} [runId: ${langRunId}]`
          );
        }
