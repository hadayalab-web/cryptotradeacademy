          ...
          if (ENABLE_TELEGRAM) {
            ...
            try {
              if (process.env[channelIdEnvVar]) {
                ...
                if (regularSendResult && (regularSendResult.message_id || regularSendResult?.result?.message_id)) {
                  sendSuccess = true;
                  console.log(`[Telegram] REGULAR message sent to ${series}/${marketCode} (${targetLang})`);
                } else {
                  console.error(`[Telegram] REGULAR message send failed for ${series}/${marketCode} (${targetLang}): sendMessageToChannel returned undefined or invalid result`);
                }
                ...
                if (regularSendResult && (regularSendResult.message_id || regularSendResult?.result?.message_id)) {
                  ...
                  console.error(`[Telegram] REGULAR message send failed for default channel (${targetLang}): sendMessage returned undefined or invalid result`);
                }
              } else {
                console.error(`[Telegram] ❌ No channel ID configured for ${series}/${marketCode} (env: ${channelIdEnvVar}) or TELEGRAM_CHAT_ID. Language: ${targetLang}`);
              }
            } catch (sendError) {
              console.error(`[Telegram] ❌ Error sending REGULAR message to ${targetLang} (${series}/${marketCode}):`, sendError.message);
              console.error(`[Telegram] Error stack:`, sendError.stack);
              // エラーが発生してもログ記録は続行
            }
            const telegramMessageId = regularSendResult?.message_id || regularSendResult?.raw?.result?.message_id || regularSendResult?.result?.message_id;
          ...
          if (ENABLE_TELEGRAM) {
            ...
            if (minimalChatId) {
              try {
                ...
                if (minimalSendResult && (minimalSendResult.message_id || minimalSendResult?.result?.message_id)) {
                  console.log(`[Free Version] ✅ Sent successfully to ${targetLang} (${langCodeForEnv}):`, minimalSendResult?.message_id || minimalSendResult?.result?.message_id || 'N/A');
                  console.log(`[MINIMAL] Successfully sent to ${targetLang}`);
                } else {
                  console.error(`[Free Version] ❌ Send failed for ${targetLang} (${langCodeForEnv}): sendMessageToAsset returned undefined or invalid result`);
                  console.error(`[MINIMAL] Failed to send to ${targetLang}`);
                }
              } catch (sendError) {
                console.error(`[Free Version] ❌ Error sending MINIMAL message to ${targetLang} (${langCodeForEnv}):`, sendError.message);
                console.error(`[Free Version] Error stack:`, sendError.stack);
                console.error(`[MINIMAL] Error sending to ${targetLang}`);
              }
            } else {
              console.error(`[Free Version] ❌ No chat ID found for ${targetLang} (${langCodeForEnv}), skipping free version delivery`);
              console.error(`[MINIMAL] Skipped ${targetLang} due to missing chat ID`);
              // 環境変数の確認ログを追加
              const possibleEnvVars = [
                `TELEGRAM_CHAT_ID_MINIMAL_${langCodeForEnv}`,
                `TELEGRAM_CHAT_ID_MINIMAL_${targetLang.toUpperCase() === 'JA' ? 'JP' : targetLang.toUpperCase() === 'KO' ? 'KR' : langCodeForEnv}`,
                'TELEGRAM_CHAT_ID_MINIMAL_EN',
                'TELEGRAM_CHAT_ID_MINIMAL',
              ];
              console.error(`[Free Version] Checked environment variables: ${possibleEnvVars.join(', ')}`);
            }
          } else {
            console.warn(`[MINIMAL] ENABLE_TELEGRAM is false, skipping ${targetLang}`);
          }
        } catch (langError) {
