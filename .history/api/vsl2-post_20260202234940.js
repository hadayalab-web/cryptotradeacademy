// api/vsl2-post.js
// ⚠️ DISABLED: VSL2配信（vsl2-free-users / vsl2-last-call）は廃止。無料版チャンネル全体・個別DMともに使用しない。
// VSL1は無料版オプトイン誘導用（X/Twitterのみ）。
...
let VSL2_YOUTUBE_LINK_RAW = process.env.VSL2_YOUTUBE_LINK || process.env.VSL_YOUTUBE_LINK || VSL_REGULAR.url;
...
async function postVSL2() {
  // ⚠️ DISABLED: VSL2配信（vsl2-free-users / vsl2-last-call）は廃止
  console.warn('⚠️ postVSL2() is disabled. VSL2 delivery is discontinued.');
  return {
    success: false,
    message: 'VSL2 delivery (vsl2-free-users, vsl2-last-call) is discontinued.',
    results: { telegram: { success: false, sent: 0, total: 0, byLang: {}, config: {} } },
  };
  /* DISABLED CODE - VSL2廃止のため未使用
  try {
    const results = {
      telegram: null,
    };
...
module.exports = async (req, res) => {
  ...
  console.warn('⚠️ /api/vsl2-post endpoint is disabled. VSL2 (vsl2-free-users / vsl2-last-call) is discontinued.');
