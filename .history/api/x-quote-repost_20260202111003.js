...
async function postQuoteRepostsForLang(
  ...
  try {
    ...
    // ENはmaxDuration=120秒のため最大18人まで処理可能（約6秒/人）。他言語は60秒で最大10人
    let maxInfluencers = targetCount; // EN: 3-5, その他: 1-3
    if (lang.toLowerCase() === "en" && maxInfluencers > 18) {
      console.warn(
        `[Quote Repost] ⚠️ Limiting EN influencers from ${maxInfluencers} to 18 (120s window) [runId: ${langRunId}]`
      );
      maxInfluencers = 18; // ENは120秒枠で最大18人（投稿を伸ばす）
    } else if (lang.toLowerCase() !== "en" && maxInfluencers > 10) {
      maxInfluencers = 10; // 他言語は60秒枠で最大10人
    }
...
const handler = async (req, res) => {
  ...
  // 各言語エントリ（x-quote-repost-en.js等）は独自のdeadlineMsを渡す。ここはマルチ言語用の60秒
  const MAX_DURATION_MS = 60_000; // Vercel FunctionsのmaxDuration=60秒（EN/EN-30はvercel.jsonで120秒）
