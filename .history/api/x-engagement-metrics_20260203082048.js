  ...
  try {
    ...
    for (const post of posts) {
      ...
      if (elapsed > timeoutMs) {
        console.warn(
          `[X Engagement Metrics] ⏰ Timeout approaching, stopping metrics update (processed: ${successCount}/${posts.length})`
        );
        break;
      }
      ...
      while (retries > 0 && !success) {
        try {
          ...
          // ツイート未検出（削除・非公開等）の場合はスキップして次へ
          if (!metrics || !metrics.publicMetrics) {
            console.warn(
              `[X Engagement Metrics] ⚠️ Skipping tweet ${post.tweetId} (not found or no data)`
            );
            break; // whileを抜けてforループの次のpostへ
          }
    ...
    try {
      const { buildInfluencerDailyPerformance } = require("../services/x/influencerPerformance");
      ...
      const perfResult = await buildInfluencerDailyPerformance(dateString, posts, metricsFetcher);
...
async function generateEngagementDashboard(dateString) {
  ...
  try {
    ...
    for (const t of metrics.tweets || []) {
      ...
      try {
        ...
        if (mapping?.influencerUsername) {
          ...
          const cur = influencerAgg.get(key) || {
            username,
            lang,
            posts: 0,
            impressions: 0,
    ...
    const p80Threshold =
      influencerRows.length > 0 ? influencerRows[Math.max(0, p80Index)]?.avgER || null : null;
    ...
    console.log(`[X Engagement Metrics] ✅ Added extensions to dashboard for ${dateString}:`, {
