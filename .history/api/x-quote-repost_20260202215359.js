    ...
    for (const influencer of influencers.slice(0, maxInfluencers)) {
      ...
      try {
        ...
        if (useMinimalRegularTemplates) {
          ...
          if (variant === "G") {
            // バリアントG: ヘッドライン → Gemini(市況＋無様なペルソナ) → チラ見せ＋リンク説明 → 反論定型 → CTA(価格・トライアル・コード)
            ...
            console.log(
              `[Quote Repost] 📌 Template (variant G, Gemini hook + fixed copy): @${influencer.username} [runId: ${langRunId}]`
            );
          } else {
