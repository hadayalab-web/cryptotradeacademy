    ...
    for (const influencer of influencers.slice(0, maxInfluencers)) {
      ...
      try {
        ...
        if (useMinimalRegularTemplates) {
          ...
          if (variant === "G") {
            // バリアントG: ヘッドライン → Gemini深層心理フック → Minimal/Regularブロック → Gemini反論処理 → プロモコード
            const {
            ...
            const geminiTimeoutMs = deadlineMs
              ? Math.min(15000, Math.max(2000, deadlineMs - Date.now() - 5000))
              : 12000;
            let hookText = null;
            ...
            if (!isDryRun) {
              try {
                hookText = await withTimeout(
                  generatePsychologyHook({ lang, trapScore: reportData?.trapScore }),
                  geminiTimeoutMs
                );
                objectionText = await withTimeout(
                  generateObjectionHandling({ lang }),
                  geminiTimeoutMs
                );
              } catch (err) {
                console.warn(
                  `[Quote Repost] Gemini variant G fallback (${err.message}): @${influencer.username} [runId: ${langRunId}]`
                );
              }
            }
            const hookFallback =
            ...
            const objectionFallback =
              lang === "ja"
                ? "この情報はリアルタイムで見ないと意味がない。知っているかどうかで、勝てる相場とギャンブルの境界線が決まる。"
            ...
            quoteText = [
              ...
              parts.minimalBlock,
              parts.regularBlock,
              objectionText || objectionFallback,
              parts.promoBlock
            ].join("\n\n");
            funnelTypeUsed = "integrated";
            console.log(
              `[Quote Repost] 📌 Template (variant G, Gemini): @${influencer.username} [runId: ${langRunId}]`
            );
          } else {
