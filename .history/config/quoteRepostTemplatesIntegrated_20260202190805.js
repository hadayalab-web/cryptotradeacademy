...
const VSL_MINIMAL_URL = "https://youtu.be/OqvqngJOiXc";
const VSL_REGULAR_URL = "https://youtu.be/fXgVsKhqDjI";

/** 有料導線CTA: 50%で月$99トライアル、50%でPRO 50% OFF（x-quote-repost.jsのgetRegularFunnelCtaと同等） */
function getRegularCtaLine(lang) {
  if (Math.random() < 0.5) return getRegularTrialCta(lang).text;
  const url = `${getWhopProductUrl(lang)}?promo=${getPromoCode()}`;
  const labels = {
    en: "PRO 50% OFF",
    ja: "PRO 50%OFF",
    es: "PRO 50% OFF",
  ...
  return `🔥 ${labels[lang] || labels.en} (${(getPromoCode() || "defend50").toUpperCase()}): ${url}`;
}
...
function getIntegratedMinimalRegularQuoteTemplate(lang, options = {}) {
  ...
  // バリアントA: 「やめられない」＋ 無料スコア・有料はPRO
  const variantA = {
    en: () =>
      `Can't stop trading? We show the trap score (free). Exit map? That's in PRO. Watch: ${VSL_MINIMAL_URL} Free: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    ja: () =>
      `「待てない」で負けてない？罠スコアは無料で。出口マップはPROの奥に。視聴: ${VSL_MINIMAL_URL} 無料: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    es: () =>
  ...
  // バリアントE: 状態言語＋gut vs data（含み損・思考停止・ルールなしの罠）
  const variantE = {
    en: () =>
      `Stuck in the "just watching" loop with unrealized loss? No rules = every move a guess. Trap Score = data vs gut. Free: ${minimalUrl} ${regularCta} Watch: ${VSL_MINIMAL_URL} #BTC #TrapDefence`,
    ja: () =>
      `含み損で「見るだけ」ループ？ルールなし=都度判断の罠。Trap Scoreでdata vs gut。無料: ${minimalUrl} ${regularCta} 視聴: ${VSL_MINIMAL_URL} #BTC #TrapDefence`,
    es: () =>
