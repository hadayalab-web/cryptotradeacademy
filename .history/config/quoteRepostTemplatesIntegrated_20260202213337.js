...
/**
 * バリアントG用: ヘッドライン＋Minimal/Regularブロック＋プロモブロックのパーツを返す。
 * 引用リポストAPI側で generatePsychologyHook / generateObjectionHandling の結果を挟んで結合する。
 * 構成: ヘッドライン → [Gemini: 深層心理フック] → 無料版チラ見せ＋VSL＋Whop → 有料版チラ見せ＋VSL＋Whop → [Gemini: 反論処理] → あなたに特別に＋プロモコード
 * @param {string} lang - 言語コード
 * @param {Object} options - { influencerUsername }
 * @returns {{ headline: string, minimalBlock: string, regularBlock: string, promoBlock: string }}
 */
function getQuoteRepostGeminiStructureParts(lang, options = {}) {
  ...
  const minimalLabel = {
    ja: "無料版（Minimal Version）チラ見せ",
    en: "Free (Minimal Version) teaser",
    es: "Gratis (Minimal Version) teaser",
    "pt-br": "Grátis (Minimal Version) teaser",
    ar: "مجاني (Minimal Version) تيزر",
    ko: "무료 (Minimal Version) 티저"
  };
  const regularLabel = {
    ja: "有料版（Regular Briefing）チラ見せ",
    en: "Paid (Regular Briefing) teaser",
    es: "De pago (Regular Briefing) teaser",
    "pt-br": "Pago (Regular Briefing) teaser",
    ar: "مدفوع (Regular Briefing) تيزر",
    ko: "유료 (Regular Briefing) 티저"
  };
  const specialForYou = {
    ja: "あなたに特別に、、、",
    en: "Just for you...",
  ...
  const ml = minimalLabel[normalizedLang] || minimalLabel.en;
  const rl = regularLabel[normalizedLang] || regularLabel.en;
  const sf = specialForYou[normalizedLang] || specialForYou.en;
  ...
  const cl = codeLabel[normalizedLang] || codeLabel.en;

  const headline = getHeadline(normalizedLang);
  const minimalBlock = `${ml}\n${VSL_MINIMAL_URL}\n${minimalUrl}`;
  const regularBlock = `${rl}\n${VSL_REGULAR_URL}\n${regularCta}`;
  const promoBlock = `${sf}\n${cl} ${promoCode}\n\n#BTC #TrapDefence`;

  return { headline, minimalBlock, regularBlock, promoBlock };
}
