 ...
 * 仕様: 月額の価格感・1日トライアルでリスクゼロ・プロモコードで50%オフを簡潔に案内する。
 */
function getCtaBlock(lang, promoCode = "defend50") {
  ...
  const blocks = {
    ja: `月額$99は、含み損の0.2〜0.5%程度です。1日トライアルで中身をご確認いただき、合わなければ解約いただけます（リスクゼロ）。
ご案内：コード ${code} で50%オフ（先着50名まで）。
▼ Regular Briefing（1日無料トライアル）`,
    en: `$99/mo is about 0.2% of a $50k loss. 1-day trial—see the report, cancel if it's not for you. Risk zero.
Code ${code} = 50% off (next 50 only).
▼ Regular Briefing (1-day free trial)`,
    es: `$99/mes equivale a ~0.2% de una pérdida de $50k. Prueba de 1 día: vea el informe, cancele si no encaja. Riesgo cero.
Código ${code} = 50% off (próximos 50).
▼ Regular Briefing (prueba 1 día gratis)`,
    "pt-br": `$99/mês equivale a ~0.2% de uma perda de $50k. Teste 1 dia: veja o relatório, cancele se não encaixar. Risco zero.
Código ${code} = 50% off (próximos 50).
▼ Regular Briefing (teste 1 dia grátis)`,
    ar: `$99/شهر ≈ 0.2% من خسارة $50k. تجربة يوم—شاهد التقرير، ألغِ إن لم يناسب. خطر صفر.
كود ${code} = خصم 50% (أول 50 فقط).
▼ Regular Briefing (يوم مجاني)`,
    ko: `$99/월은 $50k 손실의 약 0.2% 수준입니다. 1일 체험 후 레포트를 보시고, 맞지 않으면 해지하시면 됩니다 (리스크 제로).
