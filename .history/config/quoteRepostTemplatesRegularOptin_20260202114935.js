...
function getRegularOptinQuoteTemplate(lang, options = {}) {
  ...
  // バリアントD: 市況悪化・ドローダウン特化（「また負けたくない」を刺す）
  const variantD = {
    en: () =>
      `Drawdown already happened. The next trap is the one that cleans out the rest. We show the score + exit map. 50% OFF (${promo}): ${VSL_REGULAR_YOUTUBE_URL} ${whopUrl} #BTC #TrapDefence`,
    ja: () =>
      `ドローダウンはもう来た。次に来るのは「戻りだと思って嵌る罠」。スコア＋出口マップはこちら。50%OFF（${promo}）: ${VSL_REGULAR_YOUTUBE_URL} ${whopUrl} #BTC #TrapDefence`,
    es: () =>
      `El drawdown ya pasó. La próxima trampa es la que limpia a los que quedan. Mostramos score + mapa salida. 50% OFF (${promo}): ${VSL_REGULAR_YOUTUBE_URL} ${whopUrl} #BTC #TrapDefence`,
    "pt-br": () =>
      `O drawdown já veio. A próxima armadilha é a que limpa o resto. Mostramos score + mapa saída. 50% OFF (${promo}): ${VSL_REGULAR_YOUTUBE_URL} ${whopUrl} #BTC #TrapDefence`,
    ar: () =>
      `الدروداون صار. الفخ الجاي هو اللي ينضف الباقي. نوري السكور + خريطة خروج. خصم 50% (${promo}): ${VSL_REGULAR_YOUTUBE_URL} ${whopUrl} #BTC #TrapDefence`,
    ko: () =>
      `드로다운은 이미 왔음. 다음 함정이 남은 사람들 다 쓸어감. 스코어+출구 맵 여기. 50% 할인 (${promo}): ${VSL_REGULAR_YOUTUBE_URL} ${whopUrl} #BTC #TrapDefence`
  };
