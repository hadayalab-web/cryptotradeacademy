...
const OBJECTION_FIXED = {
  ...
  "pt-br": "Essa info só vale em tempo real. Ter ou não define edge tradeável vs aposta pura. Teste 1 dia—veja o relatório, cancele se não encaixar. Risco zero.",
...
function getMinimalLinkExplanation(lang) {
  return MINIMAL_LINK_EXPLANATION[(lang || "en").toLowerCase().replace("_", "-")] || MINIMAL_LINK_EXPLANATION.en;
}
function getRegularLinkExplanation(lang) {
  return REGULAR_LINK_EXPLANATION[(lang || "en").toLowerCase().replace("_", "-")] || REGULAR_LINK_EXPLANATION.en;
}
