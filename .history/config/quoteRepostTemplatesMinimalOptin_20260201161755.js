...
// プレミアムアカウント想定（文字数は 280 以内で互換性確保、必要なら拡張可）
 ...
 * @returns {string} 引用リポスト用テキスト（280文字以内）
 */
function getMinimalOptinQuoteTemplate(lang, options = {}) {
  const normalizedLang = (lang || 'en').toLowerCase().replace('_', '-');
  const whopUrl = getMinimalVersionCheckoutUrl(normalizedLang, {
    utm_content: options.influencerUsername ? `influencer_${options.influencerUsername}` : 'minimal_optin',
    ...options,
...
function getAllMinimalOptinTemplates(options = {}) {
  ...
  return Object.fromEntries(langs.map((lang) => [lang, getMinimalOptinQuoteTemplate(lang, options)]));
}
