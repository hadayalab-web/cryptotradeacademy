...
function buildRegularWhopUrl(lang, options = {}) {
  ...
  const utm = options.utm_campaign ? `&utm_campaign=${encodeURIComponent(options.utm_campaign)}` : '';
...
function getRegularOptinQuoteTemplate(lang, options = {}) {
  ...
  const whopUrl = buildRegularWhopUrl(normalizedLang, {
    utm_campaign: options.influencerUsername ? `regular_influencer_${options.influencerUsername}` : 'regular_optin',
    ...options,
  });
...
function getAllRegularOptinTemplates(options = {}) {
  ...
  return Object.fromEntries(langs.map((lang) => [lang, getRegularOptinQuoteTemplate(lang, options)]));
}
