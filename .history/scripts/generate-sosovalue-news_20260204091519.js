...
async function fetchOnchainDataWithHistory() {
  try {
    ...
    const currentInflow =
      exchangeInflow?.result?.data?.[0]?.inflow_total ??
      exchangeInflow?.result?.data?.[0]?.inflow ??
      exchangeInflow?.result?.data?.[0]?.value ??
      null;
    const currentOutflow =
      exchangeOutflow?.result?.data?.[0]?.outflow_total ??
      exchangeOutflow?.result?.data?.[0]?.outflow ??
      exchangeOutflow?.result?.data?.[0]?.value ??
      null;
