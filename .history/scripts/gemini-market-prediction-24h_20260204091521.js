...
`;
  ...
  try {
    ...
    console.log(
      `📡 API URL: https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash-preview:generateContent`
    );
    console.log(
      `🔑 API Key: ${GEMINI_API_KEY.substring(0, 20)}...${GEMINI_API_KEY.substring(GEMINI_API_KEY.length - 10)}`
    );
...
async function main() {
  ...
  try {
    ...
    if (prediction.prediction24h) {
      ...
      if (pred.priceRange) {
        console.log(
          `  価格レンジ: $${pred.priceRange.min.toLocaleString()} - $${pred.priceRange.max.toLocaleString()}`
        );
      }
