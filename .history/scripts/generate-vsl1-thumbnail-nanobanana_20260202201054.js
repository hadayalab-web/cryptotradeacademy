 ...
 *
 * 実行: node scripts/generate-vsl1-thumbnail-nanobanana.js  … NanoBanana Pro（推奨）
 *       node scripts/generate-vsl1-thumbnail-nanobanana.js --fast  … 高速モデル（Flash）
...
// NanoBanana Pro（gemini-3-pro-image-preview）をデフォルト。--fast で Flash に切り替え。
const MODEL = USE_FAST ? "gemini-2.5-flash-image" : "gemini-3-pro-image-preview";

// ペルソナ分析（personaStrategy.SEGMENT_PRIORITY）に基づく人物指定: 痛みピーク・即決ミレニアル 28-35、Z世代 22-27。おっさん・中年男性にしない。
const PERSONA_CHARACTERS =
  "CRITICAL – Character design from persona analysis: Primary target is pain-peak millennials (28–35) and Z-gen (22–27). Draw the central figure as a YOUNG ADULT in their 20s or early 30s—millennial or Gen Z appearance. Do NOT draw middle-aged men, older men, or 'ossan' (おっさん) look. Include diverse genders (the trapped trader can be any gender). Viewers must see themselves as 22–35, not 40s–50s.";
...
Theme: "Stop Being the Prey — Free Trap Score" (Trap Defence BTC). Persona: a young trader (22–35, not middle-aged) stuck with unrealized loss, mentally frozen, addicted to watching—the "prey." Visual contrast: one figure paralyzed in front of a red chart and "UNREALIZED LOSS" or "-40%", a stopped clock, thought bubble "just watching"; nearby or in shadow, a subtle "whale" shape or institutional signal. The twist: a small shield or "TRAP SCORE" badge offered like a door out—minimum edition shows the door, full protocol shows the room. Mood: satirical but recognizable—someone stuck in denial who could choose the shield. No text overlays in the image; purely visual. Caption at bottom in English only: "STOP BEING THE PREY."`;
...
async function generateImage(prompt) {
  ...
  const body = {
    ...
    generationConfig: {
      responseModalities: ["IMAGE"], // 画像のみ（TEXT を含むと 400 になるモデルあり）
...
async function main() {
  ...
  console.log("  Persona: 含み損・思考停止・トレード依存症 (22–35 歳、おっさんにしない)");
