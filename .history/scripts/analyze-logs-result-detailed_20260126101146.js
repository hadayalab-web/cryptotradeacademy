...
async function analyzeLogsDetailed() {
  ...
  try {
    ...
    for (const log of logs) {
      ...
      // Quote Repostメトリクス
      if (combined.includes('quote repost') && (combined.includes('metrics') || combined.includes('impressions') || combined.includes('engagement'))) {
        quoteRepostMetricsLogs.push(log);
      }

      // エンゲージメントメトリクス記録
      if (combined.includes('record') && (combined.includes('engagement') || combined.includes('metrics'))) {
    ...
    for (const log of logs) {
      ...
      if (message.includes('nonPublicMetrics') || message.includes('organicMetrics') || 
    ...
    Array.from(tweetIds).slice(0, 10).forEach((id, idx) => {
