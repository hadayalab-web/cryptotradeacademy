    ...
    messages: [
      {
        role: "system",
        content:
          'You return ONLY valid JSON. No markdown, no code fences. Key "usernames", value array of exactly 10 real X (Twitter) usernames of English crypto/BTC influencers, each without @ (e.g. ["APompliano","VitalikButerin"]).'
      },
      {
        role: "user",
        content:
          'List exactly 10 real English crypto/BTC influencer usernames on X. Return JSON: {"usernames": ["name1","name2",...,"name10"]}'
      }
    ],
    max_tokens: 500,
    temperature: 0.2
...
async function main() {
  ...
  console.log("=== 1. Gemini (gemini-3-flash-preview) で 10 件取得 ===");
  let geminiList = [];
  try {
  ...
  console.log(
    `  ツイート1件以上: ${geminiVerification.hasTweetsCount}/${geminiVerification.total}`
  );
  console.log("");
  ...
  console.log(
    `  Gemini 10 + Grok 10 → 重複 ${overlapCount} 件 → ユニーク ${combinedRaw.length} 件`
  );
  ...
  console.log(
    `  ツイート1件以上: ${combinedVerification.hasTweetsCount}/${combinedVerification.total}`
  );
  console.log("");
