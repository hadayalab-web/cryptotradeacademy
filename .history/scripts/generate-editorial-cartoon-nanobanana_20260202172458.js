 ...
 *
 * 実行:
 *   node scripts/generate-editorial-cartoon-nanobanana.js           … 全パターン生成
...
async function main() {
  const patternId = process.argv[2]; // 省略時は全パターン
  const list = patternId ? PATTERNS.filter((p) => p.id === patternId) : PATTERNS;
  if (list.length === 0) {
    console.log("Usage: node generate-editorial-cartoon-nanobanana.js [patternId]");
