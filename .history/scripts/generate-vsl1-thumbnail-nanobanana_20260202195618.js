 ...
 *
 * YouTube サムネイル: 4MB 以内でないとアップロード不可。保存後に 4MB 超なら警告を表示。
 */
...
const YOUTUBE_THUMBNAIL_MAX_BYTES = 4 * 1024 * 1024; // 4MB（YouTube 上限）
...
async function main() {
  ...
  try {
    ...
    if (sizeBytes > YOUTUBE_THUMBNAIL_MAX_BYTES) {
      console.warn(
        "  ⚠️ YouTube サムネは 4MB 以内が必要です。現在",
        sizeMB,
        "MB。手動で圧縮するか、--fast で再生成（小さい画像になりやすい）してください。"
      );
    }
  } catch (err) {
