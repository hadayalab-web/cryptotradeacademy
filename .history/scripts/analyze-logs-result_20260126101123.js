...
async function analyzeLogs() {
  ...
  try {
    ...
    for (const log of logs) {
      ...
      // インプレッション数に関連するログ
      if (combined.includes('impressions') || combined.includes('impression_count') || combined.includes('nonpublicmetrics') || combined.includes('organicmetrics')) {
        impressionLogs.push(log);
      }

      // エンゲージメント数に関連するログ
      if (combined.includes('engagement') || combined.includes('like_count') || combined.includes('retweet_count') || combined.includes('reply_count')) {
        engagementLogs.push(log);
      }

      // Quote Repostに関連するログ
      if (combined.includes('quote repost') || combined.includes('quote repost') || combined.includes('quote_repost')) {
        quoteRepostLogs.push(log);
      }

      // メトリクスに関連するログ
      if (combined.includes('metrics') || combined.includes('tracked') || combined.includes('recorded')) {
    ...
    if (estimatedImpressions.length > 0) {
      const totalEstimatedImpressions = estimatedImpressions.reduce((sum, item) => sum + item.count, 0);
    ...
    console.log(`実際のX APIから取得したインプレッション数: ${totalActualImpressions.toLocaleString()}`);
