...
const PERSONA_DOC_PATH = path.join(__dirname, "..", "docs", "PERSONA_ANALYSIS_TRADE_ADDICTION_2026-02.md");
...
Output structure:
1. ペルソナ解析の深掘り（既存整理の強化・補足・反論があれば）
2. 狙い打つべきターゲット（誰を・どの条件で・優先順位。具体例やセグメント名）
3. 備えるべき戦略・戦術（X / Whop / メッセージ / コンテンツ / 導線など、実行可能レベルで）
Use clear headings and bullets. Be actionable.`;
...
async function main() {
  ...
  console.log("\n--- Grok ---");
  console.log(grokResult.success ? grokResult.analysis.slice(0, 500) + "..." : grokResult.error);
  console.log("\n--- Gemini ---");
  console.log(geminiResult.success ? geminiResult.analysis.slice(0, 500) + "..." : geminiResult.error);
}
