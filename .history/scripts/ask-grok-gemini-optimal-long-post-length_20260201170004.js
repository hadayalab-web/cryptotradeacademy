...
async function askGrok() {
  ...
  try {
    const res = await client.chat.completions.create({
      model: GROK_MODEL,
      messages: [
        { role: 'system', content: 'あなたはX（Twitter）アルゴリズム解析の専門家です。データと実務に基づき簡潔に答えます。' },
...
async function askGemini() {
  if (!GEMINI_API_KEY) return { success: false, error: 'GEMINI_API_KEY is not set', model: GEMINI_MODEL };
...
async function main() {
  console.log('🔄 長文ポスト最適文字数を Grok（Xアルゴリズム）と Gemini（深層心理）に質問します...\n');
