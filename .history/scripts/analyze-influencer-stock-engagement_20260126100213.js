...
async function analyzeInfluencerStockEngagement() {
  ...
  for (const lang of LANGUAGES) {
    ...
    try {
      ...
      const minEngagementRate = engagementRates.length > 0
        ? Math.min(...engagementRates)
        : 0;

      const maxEngagementRate = engagementRates.length > 0
        ? Math.max(...engagementRates)
        : 0;
      ...
      influencers.forEach((inf, idx) => {
        ...
        console.log(`    [${idx + 1}] @${inf.username}: ER=${er.toFixed(2)}%, Impressions=${imp.toLocaleString()}`);
      });
  ...
  const overallAvgER = allAvgER.length > 0
    ? allAvgER.reduce((sum, er) => sum + er, 0) / allAvgER.length
    : 0;
