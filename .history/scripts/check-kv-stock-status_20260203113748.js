...
async function checkStockStatus() {
  ...
  for (const lang of LANGUAGES) {
    try {
      ...
      console.log(
        `${status} [${lang.toUpperCase()}] ${count}人 / 目標${target}人 (${((count / target) * 100).toFixed(1)}%)`
      );
      ...
      if (count > 0 && influencers.length > 0) {
        ...
        console.log(
          `   📊 サンプル: @${sample.username || "N/A"} (tweetId: ${sample.tweetId || "N/A"})`
        );
      }
    } catch (error) {
  ...
  console.log("=".repeat(80));
  console.log(
    `合計: ${totalCount}人 / 目標${KV_TARGET_TOTAL}人（KV） (${((totalCount / KV_TARGET_TOTAL) * 100).toFixed(1)}%)`
  );
  console.log();
