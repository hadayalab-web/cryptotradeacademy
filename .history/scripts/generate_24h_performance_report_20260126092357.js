...
async function getTelegramOptIns24h() {
  ...
  try {
    ...
    const recentOptIns = freeUsers.filter(user => {
      const userObj = typeof user === 'string' ? { chatId: user, joinedAt: new Date().toISOString() } : user;
...
async function getWhopTrafficAndConversions24h(posts, metrics) {
  // Whopリンクを含む投稿を特定
  const whopPosts = posts.filter(post => 
    post.postType === 'free_report' || post.postType === 'minimal_version'
  );
...
async function main() {
  try {
    ...
    console.log(`- 総インプレッション: ${performanceReport.summary.totalImpressions.toLocaleString()}`);
    console.log(`- 総エンゲージメント: ${performanceReport.summary.totalEngagements.toLocaleString()}`);
    ...
    if (forecastReport) {
      ...
      if (forecastReport.dailyImpressions) {
        console.log(`- インプレッション: ${forecastReport.dailyImpressions.total.toLocaleString()}`);
      }
      if (forecastReport.dailyEngagements) {
        console.log(`- エンゲージメント: ${forecastReport.dailyEngagements.total.toLocaleString()}`);
      }
