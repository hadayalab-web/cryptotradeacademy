...
const minTime = times.length > 0 ? new Date(Math.min(...times.map(t => t.getTime()))) : new Date();
const maxTime = times.length > 0 ? new Date(Math.max(...times.map(t => t.getTime()))) : new Date();
...
for (const log of logs) {
  ...
  if (postType) {
    ...
    // 言語を抽出
    let lang = null;
    const langMatch = String(log.message || log.text || '').match(/for\s+(\w{2}(?:-\w{2})?)|lang[:\s]+(\w+)/i);
    if (langMatch) {
...
const metricsPattern = /impressions?[:\s]+(\d+(?:[,\-]\d+)?)|engagement[:\s]+(\d+)|likes?[:\s]+(\d+)|retweets?[:\s]+(\d+)/i;
...
for (const post of posts) {
  ...
  post.engagementRate = impressions > 0 ? ((engagements / impressions) * 100).toFixed(2) + '%' : '0%';
}
...
const whopPosts = posts.filter(p => p.postType === 'free_report' || p.postType === 'minimal_version');
...
for (const market of Object.values(byMarket)) {
  ...
  console.log(`  - インフルエンサー: ${market.influencers.length > 0 ? market.influencers.join(', ') : 'N/A'}`);
...
(async () => {
  ...
  if (forecast) {
    ...
    if (forecast.dailyImpressions) {
      ...
      console.log(`  - Minimal Version: ${forecast.dailyImpressions.minimalVersion.toLocaleString()}`);
