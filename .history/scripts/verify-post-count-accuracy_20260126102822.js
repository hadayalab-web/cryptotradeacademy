  ...
  try {
    ...
    if (metrics && metrics.tweets) {
      ...
      if (missingInSaved.length > 0) {
        console.log(`  ⚠️  Tweet IDs in metrics but not in saved posts: ${missingInSaved.join(', ')}`);
      }
      if (missingInMetrics.length > 0) {
        console.log(`  ⚠️  Tweet IDs in saved posts but not in metrics: ${missingInMetrics.join(', ')}`);
      }
    }
    ...
    if ((dailyCount || 0) !== savedPosts.length) {
      inconsistencies.push(`Daily count (${dailyCount || 0}) != Saved posts (${savedPosts.length})`);
    }
    if (metrics && metrics.tweets && metrics.tweets.length !== savedPosts.length) {
      inconsistencies.push(`Metrics count (${metrics.tweets.length}) != Saved posts (${savedPosts.length})`);
    }
