...
Example format: {"usernames": ["handle1","handle2",...]}`;
  ...
  return Array.isArray(list)
    ? list.slice(0, batch).map((u) => String(u).replace(/^@/, '').trim()).filter(Boolean)
    : [];
}
...
async function main() {
  ...
  if (!lang || !LANGS.includes(lang)) {
    console.error('Usage: node scripts/build-influencer-list-per-lang.js --lang <en|es|pt-br|ar|ja|ko> [--batch 25]');
    process.exit(1);
  }
  checkEnv();
