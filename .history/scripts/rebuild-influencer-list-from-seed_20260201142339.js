...
function loadSeed(lang) {
  ...
  return list.filter(entry => entry && (entry.username || entry.handle) && (entry.tweetId || entry.tweet_id));
}
...
async function main() {
  ...
  if (all) {
    ...
    if (!loadSeed(lang)) {
      console.error(`No seed file for lang=${lang}. Create data/influencers-seed/influencers-${lang}.json`);
      process.exit(1);
    }
    targetLangs.push(lang);
  } else {
    console.error('Usage: node scripts/rebuild-influencer-list-from-seed.js --lang <en|es|pt-br|ar|ja|ko> | --all');
    process.exit(1);
  }
