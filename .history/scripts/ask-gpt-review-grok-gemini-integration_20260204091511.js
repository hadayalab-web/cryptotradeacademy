...
async function reviewImplementation() {
  ...
  const generateQuoteRepostTextStart = grokClientCode.indexOf(
    "async function generateQuoteRepostText"
  );
  const generateQuoteRepostTextEnd = grokClientCode.indexOf(
    "module.exports",
...
- Gemini（gemini-3-flash-preview）で心理分析を実行
      ...
      messages: [
        {
          role: "system",
          content:
            "あなたは経験豊富なソフトウェアエンジニアです。コードレビューを丁寧に行い、実装の正確性、統合の完全性、エラーハンドリング、パフォーマンス、コード品質を評価してください。"
        },
        {
          role: "user",
          content: prompt
