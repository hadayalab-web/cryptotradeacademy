...
async function main() {
  try {
    ...
    const realtimeDecisionEnginePath = join(
      __dirname,
      "../services/breakthrough/realtimeDecisionEngine.ts"
    );
...
`;
    ...
    // 結果をファイルに保存
    const outputPath = join(__dirname, "../docs/MCP_SERVER_AI_REVIEW_REPORT.json");
...
${(gptReview.priorityFixes || [])
  .map((fix: any) => `- **[${fix.priority}]** ${fix.issue}\n  - 修正方法: ${fix.fix}`)
  .join("\n")}
...
`;
    ...
    const markdownPath = join(__dirname, "../docs/MCP_SERVER_AI_REVIEW_REPORT.md");
