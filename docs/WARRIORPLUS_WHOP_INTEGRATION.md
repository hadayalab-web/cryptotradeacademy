...
- **アクセス付与系**: `sale`, `subscr_created`, `subscr_completed`, `subscr_reactivated`
  - Whop の **Checkout Session** を作成し、URL（`purchase_url`）を返します
  - `?output=text` の場合は **URLだけをプレーンテキストで返す**（KeyGen表示向け）
- **アクセス剥奪系（可能なら自動）**: `refund`, `dispute`, `subscr_cancelled`, `subscr_refunded`, `subscr_suspended`, `subscr_ended`
...
- **Whop側に「APIで直接membershipを作成する」エンドポイントが現状見当たらない**ため、この連携は「Whopの登録/アクセス導線URLを発行する」方式です。  
  もし「購入者のメールアドレスに自動でアクセス付与（支払い無し）」をやりたい場合は、Whop側の提供機能（招待/無料付与/手動追加/API）を確認し、対応エンドポイントが判明次第ここを置き換えます。
