# GitHub Copilot Agents 依頼テキスト

## 📋 GitHub.com上で使用する依頼テキスト

以下のテキストをコピーして、GitHub.com上のIssueまたはPRコメントで使用してください。
...
詳細な分析レポート: `docs/VERCEL_ERROR_ANALYSIS_JST2100_2025-12-25.md`
...
3. テスト方法の提供（ローカルビルド、デプロイバンドル確認など）
...
```markdown
@copilot Vercelデプロイで`config/marketProfiles`モジュールが見つからないエラーについて、`vercel.json`の`includeFiles`設定を確認し、解決策を提案してください。
