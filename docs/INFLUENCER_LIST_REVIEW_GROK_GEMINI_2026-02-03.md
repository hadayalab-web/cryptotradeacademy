...
- **KV の 300 リスト**は、Grok × Gemini 抽出 → X API フィルタを何週間も回して得た**実在 tweetId のみ**のリストであり、**高インプレッション・高エンゲージメントの期待値**があり、**品質が伴っている**と判断できる（経緯: `docs/INFLUENCER_LIST_300_ORIGIN.md`）。
