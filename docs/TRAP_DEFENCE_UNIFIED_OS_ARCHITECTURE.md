...
| Emergency | 重大アラート | 復活必須 |
| CRITICAL SHIFT（開発版） | Regular 特典・転換点検知 | 構造変化の瞬間アラート |
...
CRITICAL SHIFT（開発版・Regular 特典）
└ 転換点アラート（BTC 中心 + Macro）
```

> 将来拡張: CRITICAL SHIFT は KV キー / evaluator / alert template を独立させ、`CRITICAL_SHIFT_PRO` として単独プラン化できる構造を維持する。
