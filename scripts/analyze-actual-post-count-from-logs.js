  ...
  try {
    ...
    if (successCount !== successfulTweetIds.size) {
      inconsistencies.push(
        `Success log count (${successCount}) != Unique tweet IDs (${successfulTweetIds.size})`
      );
    }

    if (successCount !== savePostIdCount) {
      inconsistencies.push(
        `Success count (${successCount}) != savePostId calls (${savePostIdCount})`
      );
    }

    if (successCount !== incrementCount) {
      inconsistencies.push(
        `Success count (${successCount}) != incrementDailyPostCount calls (${incrementCount})`
      );
    }
