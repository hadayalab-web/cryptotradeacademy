#!/usr/bin/env node
/**
 * PT-BR（ブラジル・ポルトガル語）版 Minimal & Regular のサンプル出力
 *
 * 実行: node scripts/sample-output-pt-br.js
 * ファイル出力: node scripts/sample-output-pt-br.js --out output/pt-br-sample.txt
 */
...
function generateRegularOutput() {
  return formatRegularBriefing({
    ...
    psychologicalSupport: {
      ...
      psychologicalAdvice: '✅ Estado neutro - Nenhum bloqueio mental detectado: O sentimento do mercado está equilibrado.',
    },
...
function main() {
  ...
  const output = [
    '='.repeat(80),
    '📱 Versão Gratuita (Minimal Version)',
    '='.repeat(80),
    '',
    minimalMessage,
    '',
    '='.repeat(80),
    '💎 Versão Paga (Regular Briefing)',
    '='.repeat(80),
    '',
    regularMessage,
    '',
    '='.repeat(80),
    '✅ Geração de mensagens completa',
    '='.repeat(80),
  ].join('\n');
