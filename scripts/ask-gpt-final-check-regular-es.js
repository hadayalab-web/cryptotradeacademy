#!/usr/bin/env node
/**
 * GPT-5.2に修正後の有料版（Regular Briefing）のスペイン語版（ES）のみを最終チェックしてもらうスクリプト
 */
...
const langFile = path.join(__dirname, '..', 'services', 'telegram', 'messages', 'user', 'es', 'regular.es.js');
...
async function askGPTForFinalReview() {
  try {
    const prompt = `You are reviewing the Regular Briefing (Paid) message implementation for Spanish - Latin American (ES) only.
...
- ✅ **LATAM Spanish (not Spain Spanish)**: Natural, conversational, trader-to-trader tone
...
- Spain Spanish instead of LATAM Spanish
...
**Final Summary**: APPROVED or REJECTED (with specific blocking issues only).`;
    ...
    console.log('🤖 GPT-5.2にスペイン語版（ES）の最終チェックを依頼中...\n');
...
async function main() {
  console.log('🚀 GPT-5.2 有料版（Regular Briefing）最終チェックスクリプト - ES\n');
  console.log('='.repeat(80));
  console.log('📋 チェック対象: ES (Spanish - Latin American)');
  console.log('='.repeat(80) + '\n');

  const result = await askGPTForFinalReview();

  console.log('\n' + '='.repeat(80));
  console.log('📊 GPT-5.2 最終レビュー結果（ES）');
  console.log('='.repeat(80) + '\n');
  console.log(result);
  console.log('\n');
  ...
  const outputFile = path.join(outputDir, `gpt-final-check-regular-es-${timestamp}.md`);

  fs.writeFileSync(outputFile, `# GPT-5.2 有料版（Regular Briefing）最終チェック - ES\n\n生成日時: ${new Date().toISOString()}\n\n## レビュー結果\n\n${result}\n`, 'utf-8');
