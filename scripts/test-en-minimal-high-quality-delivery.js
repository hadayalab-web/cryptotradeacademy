#!/usr/bin/env node
/**
 * English version free (Minimal High Quality) test delivery
...
/**
 * English version free message test delivery
 */
async function testEnMinimalHighQualityDelivery() {
  console.log('🚀 English version free (Minimal High Quality) test delivery\n');
  ...
  try {
    ...
    const minimalChannelId = process.env.TELEGRAM_CHAT_ID_MINIMAL_EN;
    const defaultMinimalChannelId = process.env.TELEGRAM_CHAT_ID_MINIMAL;
    console.log(`  TELEGRAM_CHAT_ID_MINIMAL_EN: ${minimalChannelId || '❌ Not set'}`);
    ...
    const result = await sendMessageToAsset(message, 'MINIMAL', 'EN');
...
if (require.main === module) {
  testEnMinimalHighQualityDelivery().catch((error) => {
    console.error('❌ An error occurred during test delivery:', error);
    process.exit(1);
  });
}

module.exports = { testEnMinimalHighQualityDelivery };
