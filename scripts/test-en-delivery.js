// scripts/test-en-delivery.js
// EN版の無料版（Minimal Version）と有料版（Regular Briefing）のテスト配信
...
// 環境変数を設定（EN版のみに配信）
process.env.LANG = 'en';
process.env.REGULAR_MULTI_LANG = 'false'; // EN版のみ
process.env.MINIMAL_MULTI_LANG = 'false'; // EN版のみ
...
/**
 * EN版の定期配信（REGULAR）テスト
 */
async function testRegularDelivery() {
  console.log('\n📧 ===== EN版 有料版（Regular Briefing）テスト配信 =====\n');
  ...
  try {
    ...
    if (res.statusCode === 200) {
      console.log('\n✅ EN版 有料版（Regular Briefing）テスト配信: 成功');
      const logs = res.getLogs();
      if (logs.length > 0) {
        console.log('📊 レスポンス:', JSON.stringify(logs[0].body, null, 2));
      }
    } else {
      console.error('\n❌ EN版 有料版（Regular Briefing）テスト配信: 失敗', `Status: ${res.statusCode}`);
      const logs = res.getLogs();
      if (logs.length > 0) {
    ...
    console.error('\n❌ EN版 有料版（Regular Briefing）テスト配信: エラー', error.message);
...
/**
 * EN版の無料版（MINIMAL）テスト
 * 注意: MINIMALはisRegularSlotがtrueの場合のみ配信されるため、force=trueでも配信されない可能性があります
 */
async function testMinimalDelivery() {
  console.log('\n🆓 ===== EN版 無料版（Minimal Version）テスト配信 =====\n');
  ...
  try {
    ...
    if (res.statusCode === 200) {
      console.log('\n✅ EN版 無料版（Minimal Version）テスト配信: 成功');
      const logs = res.getLogs();
      if (logs.length > 0) {
        console.log('📊 レスポンス:', JSON.stringify(logs[0].body, null, 2));
      }
    } else {
      console.error('\n❌ EN版 無料版（Minimal Version）テスト配信: 失敗', `Status: ${res.statusCode}`);
      const logs = res.getLogs();
      if (logs.length > 0) {
        console.error('📊 エラーレスポンス:', JSON.stringify(logs[0].body, null, 2));
      }
    ...
    console.error('\n❌ EN版 無料版（Minimal Version）テスト配信: エラー', error.message);
    console.error(error.stack);
  }
}
 ...
 */
async function main() {
  console.log('🚀 EN版テスト配信を開始します...\n');
  console.log('⚠️  注意: このスクリプトは実際にメールとTelegramに配信します\n');
  ...
  // 注意: MINIMALはisRegularSlotがtrueの場合のみ配信されるため、
  // force=trueでも配信されない可能性があります
