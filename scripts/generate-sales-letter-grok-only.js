...
async function main() {
  ...
  const { prompt, text, fullText, linkBlock } = await runGrokOnlySalesLetter({
    lang: langFinal,
    reportData
  });
  ...
  console.log("========== GROK (Headline → 鷲掴み → 紹介 → 反論 → Hashtags) ==========");
  console.log(text || "(no output)");
  console.log("");

  console.log("========== LINK BLOCK (Grok風定型) ==========");
  console.log(linkBlock);
  console.log("");

  console.log("========== FULL POST (Grok + Link Block) ==========");
  console.log(fullText || "(no output)");
  console.log("");
  console.log("--- Done. ---");
}
