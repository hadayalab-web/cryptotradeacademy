// scripts/test-es-delivery-actual.js
// ES版の無料版（Minimal Version）と有料版（Regular Briefing）の実際のテスト配信
...
// 環境変数を設定（ES版のみに配信）
process.env.LANG = 'es';
process.env.REGULAR_MULTI_LANG = 'false'; // ES版のみ
process.env.MINIMAL_MULTI_LANG = 'false'; // ES版のみ
...
/**
 * ES版の定期配信（REGULAR）テスト
 */
async function testRegularDelivery() {
  console.log('\n📧 ===== ES版 有料版（Regular Briefing）テスト配信 =====\n');
  ...
  try {
    ...
    console.log(`  TELEGRAM_CHAT_ID_REGULAR_ES: ${process.env.TELEGRAM_CHAT_ID_REGULAR_ES ? '設定済み' : '未設定'}`);
    console.log('');
    
    await cronHandler(req, res);
    
    if (res.statusCode === 200) {
      console.log('\n✅ ES版 有料版（Regular Briefing）テスト配信: 成功');
      const logs = res.getLogs();
      if (logs.length > 0) {
        console.log('📊 レスポンス:', JSON.stringify(logs[0].body, null, 2));
      }
    } else {
      console.error('\n❌ ES版 有料版（Regular Briefing）テスト配信: 失敗', `Status: ${res.statusCode}`);
      const logs = res.getLogs();
      if (logs.length > 0) {
    ...
    console.error('\n❌ ES版 有料版（Regular Briefing）テスト配信: エラー', error.message);
...
async function testMinimalDelivery() {
  ...
  try {
    ...
    console.log(`  TELEGRAM_CHAT_ID_MINIMAL_ES: ${process.env.TELEGRAM_CHAT_ID_MINIMAL_ES ? '設定済み' : '未設定'}`);
    ...
    if (res.statusCode === 200) {
      console.log('\n✅ ES版 無料版（Minimal Version）テスト配信: 成功');
      const logs = res.getLogs();
      if (logs.length > 0) {
        console.log('📊 レスポンス:', JSON.stringify(logs[0].body, null, 2));
      }
    } else {
      console.error('\n❌ ES版 無料版（Minimal Version）テスト配信: 失敗', `Status: ${res.statusCode}`);
      const logs = res.getLogs();
      if (logs.length > 0) {
        console.error('📊 エラーレスポンス:', JSON.stringify(logs[0].body, null, 2));
      }
    ...
    console.error('\n❌ ES版 無料版（Minimal Version）テスト配信: エラー', error.message);
    console.error(error.stack);
  }
}
