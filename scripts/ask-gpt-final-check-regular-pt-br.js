#!/usr/bin/env node
/**
 * GPT-5.2に修正後の有料版（Regular Briefing）のポルトガル語版（PT-BR）のみを最終チェックしてもらうスクリプト
 */
...
const langFile = path.join(__dirname, '..', 'services', 'telegram', 'messages', 'user', 'pt-br', 'regular.pt-br.js');
...
async function askGPTForFinalReview() {
  try {
    const prompt = `You are reviewing the Regular Briefing (Paid) message implementation for Portuguese - Brazilian (PT-BR) only.
...
- ✅ **Brazilian Portuguese (not Portugal Portuguese)**: Natural, conversational, trader-to-trader tone
...
- Portugal Portuguese instead of Brazilian Portuguese
...
**Final Summary**: APPROVED or REJECTED (with specific blocking issues only).`;
    ...
    console.log('🤖 GPT-5.2にポルトガル語版（PT-BR）の最終チェックを依頼中...\n');
...
async function main() {
  console.log('🚀 GPT-5.2 有料版（Regular Briefing）最終チェックスクリプト - PT-BR\n');
  console.log('='.repeat(80));
  console.log('📋 チェック対象: PT-BR (Portuguese - Brazilian)');
  console.log('='.repeat(80) + '\n');

  const result = await askGPTForFinalReview();

  console.log('\n' + '='.repeat(80));
  console.log('📊 GPT-5.2 最終レビュー結果（PT-BR）');
  console.log('='.repeat(80) + '\n');
  console.log(result);
  console.log('\n');
  ...
  const outputFile = path.join(outputDir, `gpt-final-check-regular-pt-br-${timestamp}.md`);

  fs.writeFileSync(outputFile, `# GPT-5.2 有料版（Regular Briefing）最終チェック - PT-BR\n\n生成日時: ${new Date().toISOString()}\n\n## レビュー結果\n\n${result}\n`, 'utf-8');
