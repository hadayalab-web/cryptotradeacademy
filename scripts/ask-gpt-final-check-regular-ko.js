#!/usr/bin/env node
/**
 * GPT-5.2に修正後の有料版（Regular Briefing）の韓国語版（KO）のみを最終チェックしてもらうスクリプト
 */
...
const langFile = path.join(__dirname, '..', 'services', 'telegram', 'messages', 'user', 'ko', 'regular.ko.js');
...
async function askGPTForFinalReview() {
  try {
    const prompt = `You are reviewing the Regular Briefing (Paid) message implementation for Korean (KO) only.
...
- ✅ **Natural Korean**: Conversational, trader-to-trader tone (not corporate/report)
...
**Final Summary**: APPROVED or REJECTED (with specific blocking issues only).`;
    ...
    console.log('🤖 GPT-5.2に韓国語版（KO）の最終チェックを依頼中...\n');
...
async function main() {
  console.log('🚀 GPT-5.2 有料版（Regular Briefing）最終チェックスクリプト - KO\n');
  console.log('='.repeat(80));
  console.log('📋 チェック対象: KO (Korean)');
  console.log('='.repeat(80) + '\n');

  const result = await askGPTForFinalReview();

  console.log('\n' + '='.repeat(80));
  console.log('📊 GPT-5.2 最終レビュー結果（KO）');
  console.log('='.repeat(80) + '\n');
  console.log(result);
  console.log('\n');
  ...
  const outputFile = path.join(outputDir, `gpt-final-check-regular-ko-${timestamp}.md`);

  fs.writeFileSync(outputFile, `# GPT-5.2 有料版（Regular Briefing）最終チェック - KO\n\n生成日時: ${new Date().toISOString()}\n\n## レビュー結果\n\n${result}\n`, 'utf-8');
