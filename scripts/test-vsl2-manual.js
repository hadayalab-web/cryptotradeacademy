...
// VSL2無料版ユーザー配信の手動テストスクリプト

require('dotenv').config();
const path = require('path');

// VSL2配信関数を直接インポート
const vsl2FreeUsers = require('../api/vsl2-free-users');
const { loadFreeUsers, getFreeUsersForVSL2 } = require('../services/free-users/manager');

async function testVSL2FreeUsers() {
  console.log('🧪 VSL2無料版ユーザー配信の手動テスト開始...\n');
  
  // 環境変数チェック
  console.log('📋 環境変数チェック:');
  console.log(`  VSL2_YOUTUBE_LINK: ${process.env.VSL2_YOUTUBE_LINK || '❌ 未設定'}`);
  console.log(`  VSL_YOUTUBE_LINK: ${process.env.VSL_YOUTUBE_LINK || '❌ 未設定'}`);
  console.log(`  WHOP_PRODUCT_URL_EN: ${process.env.WHOP_PRODUCT_URL_EN || '❌ 未設定'}`);
  ...
  try {
    console.log('🚀 VSL2配信を実行中...\n');
    await vsl2FreeUsers(req, res);
    
    if (res.statusCode === 200) {
      console.log('\n✅ VSL2配信テスト完了！');
      if (res.body && res.body.sent > 0) {
        console.log(`📱 ${res.body.sent}件のVSL2を送信しました。`);
        console.log('📱 対象ユーザーのTelegramを確認してください。');
      } else {
        console.log('ℹ️ 配信対象ユーザーがいませんでした。');
      }
    } else {
