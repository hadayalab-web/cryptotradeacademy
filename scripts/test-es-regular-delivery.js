#!/usr/bin/env node
/**
 * ES版（スペイン語）有料版（Regular Briefing）Telegram配信テスト
...
const TEST_DATA = {
  ...
  psychologicalSupport: {
    ...
    psychologicalAdvice: 'Las condiciones del mercado son relativamente estables. Mantén la disciplina',
    mentalNote: 'La paciencia no es debilidad—es fuerza estratégica. Los mejores traders saben cuándo no operar.',
  },
 ...
 * スペイン語版有料版メッセージテスト配信
 */
async function testEsRegularDelivery() {
  console.log('🚀 Spanish Paid Version (Regular Briefing) Test Delivery\n');
  console.log('='.repeat(80));
  console.log('⚠️  Warning: This script will actually send a message to Telegram');
  console.log('⚠️  This script sends ONLY to Spanish (ES) channel');
  ...
  try {
    ...
    const { formatRegularBriefing } = require(
      '../services/telegram/messages/user/es/regular.es'
    );
    ...
    const regularChannelId = process.env.TELEGRAM_CHAT_ID_REGULAR_ES || process.env.TELEGRAM_CHAT_ID_BTC_ES;
    console.log(`  TELEGRAM_CHAT_ID_REGULAR_ES: ${regularChannelId || '❌ Not Set'}`);
    console.log(`  TELEGRAM_CHAT_ID_BTC_ES: ${process.env.TELEGRAM_CHAT_ID_BTC_ES || '❌ Not Set'}`);
    console.log(`  TELEGRAM_BOT_TOKEN: ${process.env.TELEGRAM_BOT_TOKEN ? '✅ Set' : '❌ Not Set'}`);
    console.log('');

    // Telegramに送信
    console.log('📤 Sending to Telegram...\n');
    const result = await sendMessageToChannel(message, 'BTC', 'ES');
...
if (require.main === module) {
  testEsRegularDelivery().catch((error) => {
    console.error('❌ Error during test delivery:', error);
    process.exit(1);
  });
}

module.exports = { testEsRegularDelivery };
