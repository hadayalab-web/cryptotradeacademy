    ...
    try {
      ...
      if (content) {
        ...
        const impressions =
          ...
          0;
        const engagements =
          (content.publicMetrics.like_count || 0) +
        ...
        const hasCallToAction =
          /(click|visit|get|join|start|subscribe|upgrade|buy|purchase|download|sign|register|enroll|try|learn|discover|explore|check|see|watch|read|listen|follow|share|reply|comment|like|retweet|quote)/i.test(
            content.text
          );
        ...
        console.log(
          `      インプレッション: ${impressions.toLocaleString()}, エンゲージメント: ${engagements}, エンゲージメント率: ${engagementRate.toFixed(3)}%`
        );
