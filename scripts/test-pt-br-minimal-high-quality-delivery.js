#!/usr/bin/env node
/**
 * Portuguese (PT-BR) version free (Minimal High Quality) test delivery
...
/**
 * Portuguese (PT-BR) version free message test delivery
 */
async function testPtBrMinimalHighQualityDelivery() {
  console.log('🚀 Portuguese (PT-BR) version free (Minimal High Quality) test delivery\n');
  ...
  try {
    ...
    const { formatMinimalHighQualityBriefing } = require(
      '../services/telegram/messages/user/pt-br/minimal-high-quality.pt-br'
    );
    ...
    const minimalChannelId = process.env.TELEGRAM_CHAT_ID_MINIMAL_PT_BR || process.env.TELEGRAM_CHAT_ID_MINIMAL_PTBR;
    const defaultMinimalChannelId = process.env.TELEGRAM_CHAT_ID_MINIMAL;
    console.log(`  TELEGRAM_CHAT_ID_MINIMAL_PT_BR: ${minimalChannelId || '❌ Not set'}`);
    ...
    const result = await sendMessageToAsset(message, 'MINIMAL', 'PT_BR');
...
if (require.main === module) {
  testPtBrMinimalHighQualityDelivery().catch((error) => {
    console.error('❌ An error occurred during test delivery:', error);
    process.exit(1);
  });
}

module.exports = { testPtBrMinimalHighQualityDelivery };
