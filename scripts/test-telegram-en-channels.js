...
Timestamp: ${new Date().toISOString()}`;
      ...
      const freeMessageId = freeResult?.result?.message_id || freeResult?.message_id || 'N/A';
      const freeChatTitle = freeResult?.result?.chat?.title || freeResult?.chat?.title || freeResult?.result?.sender_chat?.title || 'N/A';
