#!/usr/bin/env node
/**
 * GPT-5.2に修正後の有料版（Regular Briefing）のアラビア語版（AR）のみを最終チェックしてもらうスクリプト
 */
...
const langFile = path.join(__dirname, '..', 'services', 'telegram', 'messages', 'user', 'ar', 'regular.ar.js');
...
async function askGPTForFinalReview() {
  try {
    const prompt = `You are reviewing the Regular Briefing (Paid) message implementation for Arabic - Dubai/Gulf (AR) only.
...
- ✅ **Gulf Arabic (not generic MSA)**: Natural, conversational, trader-to-trader tone
...
- Generic MSA instead of Gulf Arabic
...
**Final Summary**: APPROVED or REJECTED (with specific blocking issues only).`;
    ...
    console.log('🤖 GPT-5.2にアラビア語版（AR）の最終チェックを依頼中...\n');
...
async function main() {
  console.log('🚀 GPT-5.2 有料版（Regular Briefing）最終チェックスクリプト - AR\n');
  console.log('='.repeat(80));
  console.log('📋 チェック対象: AR (Arabic - Dubai/Gulf)');
  console.log('='.repeat(80) + '\n');

  const result = await askGPTForFinalReview();

  console.log('\n' + '='.repeat(80));
  console.log('📊 GPT-5.2 最終レビュー結果（AR）');
  console.log('='.repeat(80) + '\n');
  console.log(result);
  console.log('\n');
  ...
  const outputFile = path.join(outputDir, `gpt-final-check-regular-ar-${timestamp}.md`);

  fs.writeFileSync(outputFile, `# GPT-5.2 有料版（Regular Briefing）最終チェック - AR\n\n生成日時: ${new Date().toISOString()}\n\n## レビュー結果\n\n${result}\n`, 'utf-8');
