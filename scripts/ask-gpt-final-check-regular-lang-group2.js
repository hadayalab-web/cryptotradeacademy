#!/usr/bin/env node
/**
 * GPT-5.2に修正後の有料版（Regular Briefing）の言語グループ2（AR, JA, KO）を最終チェックしてもらうスクリプト
 */
...
async function askGPTForFinalReview() {
  try {
    const prompt = `You are reviewing the Regular Briefing (Paid) message implementations for 3 languages: AR, JA, KO.
...
- ✅ **Language-specific**: Gulf Arabic (not generic MSA), natural Japanese/Korean
...
- Native expression is fundamentally broken (e.g., generic MSA instead of Gulf Arabic)
...
${designGuidelines.substring(0, 3000)}

## Grok × Gemini Analysis:

${grokGeminiAnalysis.substring(0, 3000)}
...
**Final Summary**: List which languages are APPROVED and which are REJECTED (with specific blocking issues only).`;
    ...
    console.log('🤖 GPT-5.2に言語グループ2（AR, JA, KO）の最終チェックを依頼中...\n');
...
async function main() {
  console.log('🚀 GPT-5.2 有料版（Regular Briefing）最終チェックスクリプト - グループ2\n');
  console.log('='.repeat(80));
  console.log('📋 チェック対象:');
  console.log('  1. AR (Arabic - Dubai/Gulf)');
  console.log('  2. JA (Japanese)');
  console.log('  3. KO (Korean)');
  console.log('='.repeat(80));
  ...
  console.log('\n' + '='.repeat(80));
  console.log('📊 GPT-5.2 最終レビュー結果（グループ2）');
  console.log('='.repeat(80) + '\n');
  console.log(result);
  console.log('\n');
  ...
  const outputFile = path.join(outputDir, `gpt-final-check-regular-group2-${timestamp}.md`);

  const outputContent = `# GPT-5.2 有料版（Regular Briefing）最終チェック - グループ2（AR, JA, KO）
