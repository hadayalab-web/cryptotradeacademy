      ...
      messages: [
        {
          role: "system",
          content:
            "あなたはX（Twitter）アルゴリズムの専門家です。2026年の最新動向を分析し、エンゲージメント率を最大化するための戦略を提供します。"
        },
        {
          role: "user",
          content: prompt
  ...
  try {
    ...
    const integratedResult = {
      ...
      integratedStrategy: {
        ...
        timingOptimization:
          grokAnalysis.algorithmInsights?.timingOptimization || "ピーク時間（UTC 0,1,20,21）に投稿",
        formatOptimization:
        ...
        cvrOptimization: {
          telegramOptIn:
            geminiAnalysis.cvrAlgorithm?.telegramOptIn?.strategy ||
            "Telegram Deep Linkを明確に表示し、無料価値を強調",
          whopConversion:
            geminiAnalysis.cvrAlgorithm?.whopConversion?.strategy ||
...
`;
    ...
    console.log(
      `  質問CTA: ${integratedResult.integratedStrategy.contentOptimization.questionCTA}`
    );
    console.log(`  リンク: ${integratedResult.integratedStrategy.contentOptimization.links}`);
    console.log(
      `  ハッシュタグ: ${integratedResult.integratedStrategy.contentOptimization.hashtags}`
    );
...
module.exports = {
  ...
  analyzeEngagementCVRWithGemini
};
