#!/usr/bin/env node
/**
 * Korean (KO) version free (Minimal High Quality) test delivery
...
/**
 * Korean (KO) version free message test delivery
 */
async function testKoMinimalHighQualityDelivery() {
  console.log('🚀 Korean (KO) version free (Minimal High Quality) test delivery\n');
  ...
  try {
    ...
    const { formatMinimalHighQualityBriefing } = require(
      '../services/telegram/messages/user/ko/minimal-high-quality.ko'
    );
    ...
    const minimalChannelId = process.env.TELEGRAM_CHAT_ID_MINIMAL_KO || process.env.TELEGRAM_CHAT_ID_MINIMAL_KR;
    const defaultMinimalChannelId = process.env.TELEGRAM_CHAT_ID_MINIMAL;
    console.log(`  TELEGRAM_CHAT_ID_MINIMAL_KO: ${minimalChannelId || '❌ Not set'}`);
    ...
    const result = await sendMessageToAsset(message, 'MINIMAL', 'KO');
...
if (require.main === module) {
  testKoMinimalHighQualityDelivery().catch((error) => {
    console.error('❌ An error occurred during test delivery:', error);
    process.exit(1);
  });
}

module.exports = { testKoMinimalHighQualityDelivery };
