#!/usr/bin/env node
/**
 * GPT-5.2に修正後の有料版（Regular Briefing）の言語グループ1（EN, ES, PT-BR）を最終チェックしてもらうスクリプト
 */
...
async function askGPTForFinalReview() {
  try {
    const prompt = `You are reviewing the Regular Briefing (Paid) message implementations for 3 languages: EN, ES, PT-BR.
...
- ✅ **Language-specific**: LATAM Spanish (not Spain), Brazilian Portuguese (not Portugal), natural English
...
**Final Summary**: List which languages are APPROVED and which are REJECTED (with specific blocking issues only).`;
    ...
    console.log('🤖 GPT-5.2に言語グループ1（EN, ES, PT-BR）の最終チェックを依頼中...\n');
...
async function main() {
  console.log('🚀 GPT-5.2 有料版（Regular Briefing）最終チェックスクリプト - グループ1\n');
  ...
  console.log('\n' + '='.repeat(80));
  console.log('📊 GPT-5.2 最終レビュー結果（グループ1）');
  console.log('='.repeat(80) + '\n');
  console.log(result);
  console.log('\n');
  ...
  const outputFile = path.join(outputDir, `gpt-final-check-regular-group1-${timestamp}.md`);

  const outputContent = `# GPT-5.2 有料版（Regular Briefing）最終チェック - グループ1（EN, ES, PT-BR）
