...
function verifyWebhookSignature(signature, body, timestamp) {
  ...
  if (!X_API_CONSUMER_KEY_SECRET) {
    ...
    console.warn(
      "[X Webhook] X_API_CONSUMER_KEY_SECRET not set, skipping signature verification (development mode)"
    );
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[X Webhook] ⚠️ Base64 decode failed, falling back to string comparison:",
        base64Error.message
      );
...
async function handleSingleRetweetEvent(retweetEvent) {
  try {
    ...
    const tweetId =
      retweetEvent?.target_object?.id_str ||
    ...
    if (!tweetId) {
      console.warn(
        "[X Webhook] Retweet event missing tweetId:",
        JSON.stringify(retweetEvent, null, 2).substring(0, 200)
      );
      return;
    }
...
async function handleSingleReplyEvent(replyTweet) {
  try {
    ...
    console.log(
      `[X Webhook] ✅ Reply event: replyToTweetId=${replyToTweetId}, replyTweetId=${replyTweetId}, userId=${userId}, text=${replyText?.substring(0, 50)}`
    );
...
async function handleReplyEvent(event) {
  ...
  // リプライイベントのみをフィルタリング
  const replyEvents = event.tweet_create_events.filter((e) => e.in_reply_to_status_id_str);
...
async function detectViralPost(tweetId, stats) {
  ...
  if (totalEngagement >= 100) {
    console.log(
      `[X Webhook] 🚀 VIRAL POST DETECTED: tweetId=${tweetId}, engagement=${totalEngagement}`
    );
  ...
  try {
    ...
    if (kv && typeof kv.set === "function") {
      ...
      const logData = {
        ...
        hasBody: !!req.body,
        headers: {
          "x-twitter-webhooks-signature": req.headers["x-twitter-webhooks-signature"]
            ? "present"
            : "missing",
      ...
      kv.set(logKey, logData, { ex: 86400 * 7 }).catch((error) => {
        console.warn(
          "[X Webhook] ⚠️ Failed to save access log to KV (non-critical):",
          error.message
        );
      });
    }
...
async function getRawBodyFromRequest(req) {
  try {
    ...
    if (typeof req.body === "string") {
      ...
      console.warn(
        "[X Webhook] ⚠️ Falling back to JSON.stringify (not ideal for signature verification)"
      );
...
async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      console.log(
        "[X Webhook] ✅ CRC verification successful for token:",
        crcToken.substring(0, 10) + "..."
      );
  ...
  if (req.method === "POST") {
    try {
      ...
      const signature =
        ...
        req.headers["X-Twitter-Webhook-Signature"];
      const timestamp =
        req.headers["x-twitter-request-timestamp"] || req.headers["X-Twitter-Request-Timestamp"];
      ...
      if (isReplayJobStatus) {
        console.log(
          "[X Webhook] 📋 Replay job status event detected, skipping signature verification"
        );
        ...
        if (isReplayJobStatus) {
          ...
          console.error(
            "[X Webhook] ❌ CRITICAL: Missing signature or timestamp headers in production"
          );
