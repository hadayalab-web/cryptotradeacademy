...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // P0 FIX: deadlineMsを設定（Vercel maxDuration=300。起動〜KV取得で約15s消費するため実質285sで処理）
    const MAX_DURATION_MS = 290_000; // 290秒（10秒マージン）
    const deadlineMs = Date.now() + MAX_DURATION_MS - 2000;
    ...
    console.log(
      `[QuoteRepost-PT-BR] ✅ 完了: ${successCount}/${totalCount} 投稿成功, ${dryRunCount}/${totalCount} ドライラン成功 [runId: ${runId}]`
    );
    console.log(
      `[QuoteRepost-PT-BR] 📊 Success determination: successCount=${successCount}, dryRunCount=${dryRunCount}, totalCount=${totalCount}, overallSuccess=${overallSuccess} [runId: ${runId}]`
    );
