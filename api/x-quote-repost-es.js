...
module.exports = async (req, res) => {
  const runId = `qr-es-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
  
  try {
    console.log(`[QuoteRepost-ES] 実行開始: ${new Date().toISOString()} [runId: ${runId}]`);
    ...
    if (!force) {
      ...
      if (!langs.includes('es') || type !== 'quote') {
        console.log(`[QuoteRepost-ES] ⏰ SKIPPED: Not peak time for ES quote reposts [runId: ${runId}, currentHour: ${currentHour} UTC]`);
        return res.status(200).json({
          success: true,
          skipped: true,
          reason: 'not_peak_time',
          currentHour,
          message: 'Not peak time for ES quote reposts. Use ?force=true to override.',
          runId,
          timestamp: new Date().toISOString(),
        });
      }
      console.log(`[QuoteRepost-ES] ✅ Peak time confirmed: UTC ${currentHour}:00 [runId: ${runId}]`);
    } else {
      console.log(`[QuoteRepost-ES] 🔧 Force mode enabled - skipping time check [runId: ${runId}]`);
    }
    ...
    if (!reportData || !reportData.trapScore || !reportData.priceUsd) {
      console.log('[QuoteRepost-ES] Fetching latest market data...');
      try {
        ...
        console.error('[QuoteRepost-ES] ❌ Error fetching market data:', fetchError.message);
        // デフォルト値を使用して続行
        reportData = {
          trapScore: 0,
          priceUsd: 89077,
    ...
    console.log(`[QuoteRepost-ES] 📊 Results analysis [runId: ${runId}]:`, {
    ...
    console.log(`[QuoteRepost-ES] ✅ 完了: ${successCount}/${totalCount} 投稿成功, ${dryRunCount}/${totalCount} ドライラン成功 [runId: ${runId}]`);
    console.log(`[QuoteRepost-ES] 📊 Success determination: successCount=${successCount}, dryRunCount=${dryRunCount}, totalCount=${totalCount}, overallSuccess=${overallSuccess} [runId: ${runId}]`);
    ...
    console.error(`[QuoteRepost-ES] ❌ エラー [runId: ${runId}]:`, error.message);
    console.error(`[QuoteRepost-ES] Stack:`, error.stack);
