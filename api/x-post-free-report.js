...
const QUOTE_REPOST_TEMPLATES = {
  ...
  ja: (trapScore, priceUsd, change24h, deepLink, exchangeNetflow = null, whaleRatio = null) => {
    ...
    const changeStr =
      change24h != null ? `${change24h >= 0 ? "+" : ""}${change24h.toFixed(2)}%` : "";
    const question =
      trapScore <= 25 ? "待機中？それとも取引中？" : "資本保護？それとも追いかけ中？";
  ...
  es: (trapScore, priceUsd, change24h, deepLink, exchangeNetflow = null, whaleRatio = null) => {
    ...
    const changeStr =
      change24h != null ? `${change24h >= 0 ? "+" : ""}${change24h.toFixed(2)}%` : "";
    const question =
      trapScore <= 25 ? "¿Esperando o operando?" : "¿Protegiendo capital o persiguiendo?";
  ...
  "pt-br": (
    ...
    const changeStr =
      change24h != null ? `${change24h >= 0 ? "+" : ""}${change24h.toFixed(2)}%` : "";
    const question =
      trapScore <= 25 ? "Esperando ou operando?" : "Protegendo capital ou perseguindo?";
  ...
  ko: (trapScore, priceUsd, change24h, deepLink, exchangeNetflow = null, whaleRatio = null) => {
    ...
    const changeStr =
      change24h != null ? `${change24h >= 0 ? "+" : ""}${change24h.toFixed(2)}%` : "";
    const question =
      trapScore <= 25 ? "대기 중인가요? 거래 중인가요?" : "자본 보호 중인가요? 추격 중인가요?";
...
const TWEET_TEMPLATES = {
  en: (trapScore, priceUsd, change24h, deepLink, exchangeNetflow = null, whaleRatio = null) => {
    ...
    const whaleDollarStr =
      whaleRatio && priceUsd ? `$${Math.floor((whaleRatio / 100) * priceUsd * 1000)}M+ ready` : "";
    ...
    const contradictionText =
      trapScore <= 25 && exchangeNetflow && exchangeNetflow > 0 && whaleRatio && whaleRatio > 50
...
━━━━━━━━━━━━━━━━━━━━
🔥 UPGRADE NOW: PRO ACCESS (50% OFF ${getPromoCode().toUpperCase()})
━━━━━━━━━━━━━━━━━━━━
💎 Unlock Full Access + Alerts: ${getWhopProductUrl("en")}?promo=${getPromoCode()}
🚨 Limited Time: ${getPromoCode().toUpperCase()} code expires soon!
...
━━━━━━━━━━━━━━━━━━━━
🔥 今すぐアップグレード: PRO版アクセス（50%OFF ${getPromoCode().toUpperCase()}）
━━━━━━━━━━━━━━━━━━━━
💎 フルアクセス+アラート解除: ${getWhopProductUrl("ja")}?promo=${getPromoCode()}
🚨 期間限定: ${getPromoCode().toUpperCase()}コードはまもなく期限切れ！
...
━━━━━━━━━━━━━━━━━━━━
🔥 UPGRADE AGORA: ACESSO PRO (50% OFF ${getPromoCode().toUpperCase()})
━━━━━━━━━━━━━━━━━━━━
💎 Desbloqueie Acesso Completo + Alertas: ${getWhopProductUrl("pt-br")}?promo=${getPromoCode()}
🚨 Tempo Limitado: Código ${getPromoCode().toUpperCase()} expira em breve!
...
━━━━━━━━━━━━━━━━━━━━
🔥 ترقية الآن: الوصول PRO (50% خصم ${getPromoCode().toUpperCase()})
━━━━━━━━━━━━━━━━━━━━
💎 فك قفل الوصول الكامل + التنبيهات: ${getWhopProductUrl("ar")}?promo=${getPromoCode()}
🚨 وقت محدود: كود ${getPromoCode().toUpperCase()} ينتهي قريباً!
...
━━━━━━━━━━━━━━━━━━━━
🔥 지금 업그레이드: PRO 액세스 (50% 할인 ${getPromoCode().toUpperCase()})
━━━━━━━━━━━━━━━━━━━━
💎 전체 액세스+알림 잠금 해제: ${getWhopProductUrl("ko")}?promo=${getPromoCode()}
🚨 제한 시간: ${getPromoCode().toUpperCase()} 코드 곧 만료!
...
async function postFreeReportAsThread(targetLangs, reportData) {
  ...
  if (alreadyPosted) {
    console.log(
      `[X Post Free Report] ⏰ Free report already posted today (${dateString}), skipping to avoid duplicate`
    );
  ...
  console.log(
    `[X Post Free Report] Daily post count: ${dailyPostCount} (no limit, controlled by Cron schedule)`
  );
  ...
  try {
    ...
    console.warn(
      "[X Post Free Report] Failed to get optimized content format, using default:",
      error.message
    );
  ...
  console.log(
    `[X Post Free Report] Hourly post count: ${currentHourlyPostCount}/${maxPostsPerHour}`
  );
  ...
  for (let i = 0; i < langsToPost.length; i++) {
    ...
    if (!isPeakHour) {
      ...
      if (currentHourlyCount >= peakHourLimit) {
        console.log(
          `⏰ Skipping ${lang} (not peak hour and hourly limit near: ${currentHourlyCount}/${peakHourLimit})`
        );
        continue;
      }
      console.log(
        `ℹ️ Posting ${lang} outside peak hour (${currentHour} UTC) for impression maximization`
      );
    }
    ...
    if (!checkHourlyPostLimit(currentHourlyCount, maxPostsPerHour)) {
      console.log(
        `⏰ Skipping ${lang} (hourly post limit reached: ${currentHourlyCount}/${maxPostsPerHour})`
      );
      continue;
    }
    ...
    try {
      ...
      let langMainTweet = tweetTemplate(
        trapScore,
        priceUsd,
        change24h,
        getTelegramDeepLinkWithSource(lang, "x_direct"),
        exchangeNetflow,
        whaleRatio
      );
      ...
      // Grok推奨: 2ハッシュタグ最大
      const hashtagsArray = Array.isArray(optimizedHashtags)
        ? optimizedHashtags.slice(0, 2)
        : [optimizedHashtags].slice(0, 2);
      langMainTweet = langMainTweet.replace(
        new RegExp(langHashtags.replace(/#/g, "\\#").replace(/\s+/g, ".*"), "g"),
        hashtagsArray.join(" ")
      );

      // Grok推奨: フック戦略（冒頭最適化・長文ポスト対応）
      ...
      const usePoll =
        contentFormat === "thread_with_poll" ||
      ...
      try {
        ...
        if (content) {
            ...
            try {
              ...
              if (videoBuffer) {
                ...
                if (videoMediaId) {
                  ...
                  console.log(
                    `[X Post Free Report] ✅ Video uploaded for ${lang}: ${videoMediaId}`
                  );
                  ...
                  try {
                    ...
                    console.warn(
                      `[X Post Free Report] ⚠️ Failed to record cost for video upload:`,
                      costError.message
                    );
                  }
                }
              ...
              console.warn(
                `[X Post Free Report] Failed to upload video for ${lang}:`,
                error.message
              );
            }
          }
          ...
          if (
            (contentFormat === "thread_with_image" ||
              (contentFormat === "thread_with_video" && mediaIds.length === 0)) &&
            content.imageUrl
          ) {
            try {
              ...
              if (imageBuffer) {
                ...
                if (imageMediaId) {
                  ...
                  console.log(
                    `[X Post Free Report] ✅ Image uploaded for ${lang}: ${imageMediaId}`
                  );
                  ...
                  try {
                    ...
                    console.warn(
                      `[X Post Free Report] ⚠️ Failed to record cost for image upload:`,
                      costError.message
                    );
                  }
                }
              ...
              console.warn(
                `[X Post Free Report] Failed to upload image for ${lang}:`,
                error.message
              );
            }
          }
        ...
        console.warn(
          `[X Post Free Report] Failed to get content from KV for ${lang}:`,
          error.message
        );
      }

      // メイン投稿を実行（メディアIDを添付）
      const langMainResult = await postTweet(
        langMainTweet,
      ...
      try {
        ...
        console.log(
          `[X Post Free Report] ✅ Post count incremented after successful save (main tweet)`
        );
      } catch (saveError) {
        // CRITICAL: 保存に失敗した場合は致命的エラー
        console.error(
          `[X Post Free Report] ❌ CRITICAL: Failed to save main post ID:`,
          saveError.message
        );
        throw new Error(
          `CRITICAL: Failed to save main post ID to KV: ${langMainTweetId}. Original error: ${saveError.message}`
        );
      }
      ...
      try {
        ...
        setTimeout(async () => {
          try {
            ...
            const velocityResult = await replyToTweet(velocityReply, currentMainTweetId);
            console.log(
              `[X Post Free Report] ✅ Velocity self-question posted for ${lang}: ${currentMainTweetId}`
            );
            ...
            try {
              ...
              console.warn(
                `[X Post Free Report] ⚠️ Failed to record cost for velocity reply:`,
                costError.message
              );
            }
          } catch (error) {
            console.error(
              `[X Post Free Report] ❌ CRITICAL: Failed to post velocity self-question for ${lang}:`,
              {
                error: error.message,
                stack: error.stack,
        ...
        console.warn(
          `[X Post Free Report] Failed to schedule velocity self-question for ${lang}:`,
          error.message
        );
      }
      ...
      try {
        ...
        handleUserReplies(langMainTweetId, lang, reportData, 10).catch((error) => {
          console.warn(
            `[X Post Free Report] Failed to handle user replies for ${lang}:`,
            error.message
          );
        });
        console.log(
          `[X Post Free Report] 🚀 User reply handler scheduled for tweet ${langMainTweetId} (${lang})`
        );
      } catch (error) {
        console.warn(
          `[X Post Free Report] Failed to schedule user reply handler for ${lang}:`,
          error.message
        );
      }
    ...
    for (let i = 0; i < actualReplyCount; i++) {
      ...
      if (!checkHourlyPostLimit(currentHourlyCount, maxPostsPerHour)) {
        console.log(
          `⏰ Hourly post limit reached during thread replies (${currentHourlyCount}/${maxPostsPerHour}), stopping`
        );
        break;
      }
      try {
        // Grok推奨: スレッド内容を深掘り（データポイント、インサイト、CTA）
        const threadContent = generateThreadReply(
          lang,
          i,
          replyCount,
          trapScore,
        ...
        const { getTrendyHashtags } = require("../services/x/optimization");
        const langHashtags = await getTrendyHashtags(lang, "BTC").catch(() =>
          getOptimizedHashtags(lang)
        );
        const optimizedThreadText = threadText.replace(
          /#BTC.*#TrapDefence/g,
          Array.isArray(langHashtags) ? langHashtags.join(" ") : langHashtags
        );

        // スレッドはリプライとして投稿（各言語のメイン投稿にリプライ）
        if (!langMainTweetId) {
          console.error(
            `[X Post Free Report] ❌ CRITICAL: langMainTweetId is null for ${lang}, skipping thread reply`
          );
          break;
        }
        const threadResult = await replyToTweet(optimizedThreadText, langMainTweetId);
        results.push({
          lang,
          success: true,
        ...
        console.log(
          `✅ Thread ${i + 2}/${actualReplyCount + 1} posted for ${lang}: ${threadResult.id}`
        );
        ...
        try {
          ...
          console.warn(
            `[X Post Free Report] ⚠️ Failed to record cost for thread reply:`,
            costError.message
          );
        }
        ...
        if (!threadSaveSuccess) {
          console.warn(
            `[X Post Free Report] ⚠️ Failed to save thread post ID: ${threadResult.id} (non-critical, continuing)`
          );
        }
...
const handler = async (req, res) => {
  ...
  try {
    ...
    if (!kv) {
      console.warn(
        "[X Post Free Report] ⚠️ KV storage not available - duplicate prevention may not work"
      );
    } else {
    ...
    if (!xStatus.configured) {
      console.error(
        `[X Post Free Report] ❌ X API not configured, missing: ${xStatus.missing.join(", ")}`
      );
