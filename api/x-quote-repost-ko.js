...
module.exports = async (req, res) => {
  const runId = `qr-ko-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
  
  try {
    console.log(`[QuoteRepost-KO] 実行開始: ${new Date().toISOString()} [runId: ${runId}]`);
    ...
    if (!force) {
      ...
      if (!langs.includes('ko') || type !== 'quote') {
        console.log(`[QuoteRepost-KO] ⏰ SKIPPED: Not peak time for KO quote reposts [runId: ${runId}, currentHour: ${currentHour} UTC]`);
        return res.status(200).json({
          success: true,
          skipped: true,
          reason: 'not_peak_time',
          currentHour,
          message: 'Not peak time for KO quote reposts. Use ?force=true to override.',
          runId,
          timestamp: new Date().toISOString(),
        });
      }
      console.log(`[QuoteRepost-KO] ✅ Peak time confirmed: UTC ${currentHour}:00 [runId: ${runId}]`);
    } else {
      console.log(`[QuoteRepost-KO] 🔧 Force mode enabled - skipping time check [runId: ${runId}]`);
    }
    ...
    if (!reportData || !reportData.trapScore || !reportData.priceUsd) {
      console.log('[QuoteRepost-KO] Fetching latest market data...');
      try {
        ...
        console.error('[QuoteRepost-KO] ❌ Error fetching market data:', fetchError.message);
        // デフォルト値を使用して続行
        reportData = {
          trapScore: 0,
          priceUsd: 89077,
    ...
    console.log(`[QuoteRepost-KO] 📊 Results analysis [runId: ${runId}]:`, {
    ...
    console.log(`[QuoteRepost-KO] ✅ 完了: ${successCount}/${totalCount} 投稿成功, ${dryRunCount}/${totalCount} ドライラン成功 [runId: ${runId}]`);
    console.log(`[QuoteRepost-KO] 📊 Success determination: successCount=${successCount}, dryRunCount=${dryRunCount}, totalCount=${totalCount}, overallSuccess=${overallSuccess} [runId: ${runId}]`);
    ...
    console.error(`[QuoteRepost-KO] ❌ エラー [runId: ${runId}]:`, error.message);
    console.error(`[QuoteRepost-KO] Stack:`, error.stack);
