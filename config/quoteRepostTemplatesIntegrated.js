...
// VSLリンクはGrokセールスレター（salesLetterContest getLinkBlockGrokStyle）のみで使用。統合テンプレートではWhop導線のみ。
...
/**
 * 簡易フロー用: テーマ→本文の下に置くCTAブロック（価格・無料リンク・有料リンク・ハッシュタグ）
 * @param {string} lang - 言語コード
 * @param {Object} options - { influencerUsername }
 * @returns {string}
 */
function getSimpleCtaWithLinks(lang, options = {}) {
  const normalizedLang = (lang || "en").toLowerCase().replace("_", "-");
  const minimalUrl =
    getMinimalVersionCheckoutUrl(normalizedLang, {
      utm_content: options.influencerUsername
  ...
  const regularCta = getRegularCtaLine(normalizedLang);
  const promoCode = getPromoCode && getPromoCode() ? getPromoCode().toUpperCase() : "DEFEND50";
  return `${getCtaBlock(normalizedLang, promoCode)}\n\n${minimalUrl}\n\n${regularCta}\n\n#BTC #TrapDefence`;
}
...
function getIntegratedMinimalRegularQuoteTemplate(lang, options = {}) {
  ...
  const variantA = {
    ...
    es: () =>
      `¿No puedes dejar de operar? Mostramos el trap score (gratis). ¿Mapa de salida? En Regular Briefing. Hoy en TG gratis: «Score 12/100. Mapa—» Resto en Telegram. Regular Briefing: «Ballenas cargando. Siguiente trampa—» Resto en Whop. Mira: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    "pt-br": () =>
      `Não para de operar? Mostramos o trap score (grátis). Mapa de saída? Está no Regular Briefing. Hoje no TG grátis: «Score 12/100. Mapa—» Resto no Telegram. Regular Briefing: «Baleias carregando. Próxima armadilha—» Resto no Whop. Assista: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    ar: () =>
      `ما تقدر توقف التداول؟ نوري trap score (مجاني). خريطة الخروج؟ في Regular Briefing. اليوم في TG مجاني: «Score 12/100. خريطة—» الباقي في تيليجرام. Regular Briefing: «حيتان تحمّل. الفخ الجاي—» الباقي في Whop. شوف: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    ko: () =>
      `못 참고 매매해? 트랩 스코어는 무료. 출구 맵? Regular Briefing 안에 있어. 오늘 무료 TG 한 조각: «스코어 12/100. 출구 맵—» 나머지는 TG에서. Regular Briefing: «고래 로딩. 다음 함정—» 나머지는 Whop에서. 시청: ${minimalUrl} ${regularCta} #BTC #TrapDefence`
  };

  // バリアントB: 「彼らは笑った」＋ 無料/有料TGチラ見せ（ツァイガルニク）→VSL＋Whop
  const variantB = {
    en: () =>
      `They laughed when I said "wait for the trap." Then the dump came. Score + exit map: free tier & Regular Briefing. Today's free TG: «Score 12/100. Exit map—» Rest in Telegram. Regular Briefing: «Whales loading. Next trap—» Rest in Whop. Watch: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    ja: () =>
      `「罠を待て」と言ったら笑われた。そのあとダンプが来た。スコア＋出口は無料と有料で。今日の無料TG: «スコア12/100。出口マップ—» 続きはTGで。有料: «クジラ積み中。次の罠—» 続きはWhopで。視聴: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    es: () =>
      `Se rieron cuando dije "espera la trampa." Luego vino el dump. Score + mapa salida: gratis y Regular Briefing. Hoy TG gratis: «Score 12/100. Mapa—» Resto en Telegram. Regular Briefing: «Ballenas. Siguiente trampa—» Resto en Whop. Mira: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    "pt-br": () =>
      `Riram quando falei "espere a armadilha." Veio o dump. Score + mapa saída: grátis e Regular Briefing. Hoje TG grátis: «Score 12/100. Mapa—» Resto no Telegram. Regular Briefing: «Baleias. Próxima armadilha—» Resto no Whop. Assista: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    ar: () =>
      `ضحكوا لما قلت "استنى الفخ." بعدين جت الـ dump. سكور + خريطة خروج: مجاني و Regular Briefing. اليوم TG مجاني: «Score 12/100. خريطة—» الباقي في تيليجرام. Regular Briefing: «حيتان. الفخ الجاي—» الباقي في Whop. شوف: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    ko: () =>
      `"함정 기다려" 하니까 비웃더라. 그다음 덤프 왔음. 스코어+출구: 무료랑 Regular Briefing. 오늘 무료 TG: «스코어 12/100. 출구 맵—» 나머지는 TG. Regular Briefing: «고래. 다음 함정—» 나머지는 Whop. 시청: ${minimalUrl} ${regularCta} #BTC #TrapDefence`
  };

  // バリアントC: 「90%が嵌る罠」＋ 無料でスコア・有料で全文
  const variantC = {
    en: () =>
      `The trap 90% of traders fall into. Score free; "where to exit" in Regular Briefing. Watch:  Free: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    ja: () =>
      `トレーダー90%が嵌る罠。スコアは無料。「どこで出口」は有料ブリーフで。視聴:  無料: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    es: () =>
      `La trampa en la que caen 90% de traders. Score gratis; "dónde salir" en Regular Briefing. Mira:  Gratis: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    "pt-br": () =>
      `A armadilha em que 90% dos traders caem. Score grátis; "onde sair" no Regular Briefing. Assista:  Grátis: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    ar: () =>
      `الفخ اللي 90% من المتداولين يقعوا فيه. سكور مجاني؛ "وين تخرج" في Regular Briefing. شوف:  مجاني: ${minimalUrl} ${regularCta} #BTC #TrapDefence`,
    ko: () =>
      `트레이더 90%가 걸리는 함정. 스코어 무료. "어디서 출구"는 Regular Briefing에. 시청:  무료: ${minimalUrl} ${regularCta} #BTC #TrapDefence`
  };

  // バリアントD: ドローダウン特化＋TGチラ見せ→VSL＋Whop
  const variantD = {
    en: () =>
      `Market bleeding? Don't jump in without the trap score. Free score; exit map in Regular Briefing. Today's free TG: «Score 12/100. Exit map—» Rest in Telegram. Regular Briefing: «Next trap. On-chain—» Rest in Whop.  ${minimalUrl} ${regularCta} #BTC #TrapScore #RiskOff #TrapDefence`,
    ja: () =>
      `相場が血の海のとき、スコア見ずに飛び込むな。無料スコア；出口マップは有料ブリーフで。今日の無料TG: «スコア12/100。出口マップ—» 続きはTGで。有料: «次の罠。オンチェーン—» 続きはWhopで。 ${minimalUrl} ${regularCta} #BTC #TrapScore #RiskOff #TrapDefence`,
    es: () =>
      `¿Mercado en rojo? No entres sin el trap score. Score gratis; mapa salida en Regular Briefing. Hoy TG gratis: «Score 12/100. Mapa—» Resto en Telegram. Regular Briefing: «Próxima trampa. On-chain—» Resto en Whop.  ${minimalUrl} ${regularCta} #BTC #TrapScore #RiskOff #TrapDefence`,
    "pt-br": () =>
      `Mercado sangrando? Não entre sem o trap score. Score grátis; mapa saída no Regular Briefing. Hoje TG grátis: «Score 12/100. Mapa—» Resto no Telegram. Regular Briefing: «Próxima armadilha. On-chain—» Resto no Whop.  ${minimalUrl} ${regularCta} #BTC #TrapScore #RiskOff #TrapDefence`,
    ar: () =>
      `السوق ينزف؟ لا تدخل بدون trap score. سكور مجاني؛ خريطة خروج في Regular Briefing. اليوم TG مجاني: «Score 12/100. خريطة—» الباقي في تيليجرام. Regular Briefing: «الفخ الجاي. On-chain—» الباقي في Whop.  ${minimalUrl} ${regularCta} #BTC #TrapScore #RiskOff #TrapDefence`,
    ko: () =>
      `시장 피터지는데 스코어 안 보고 들어가? 무료 스코어；출구 맵은 Regular Briefing에. 오늘 무료 TG: «스코어 12/100. 출구 맵—» 나머지는 TG. Regular Briefing: «다음 함정. On-chain—» 나머지는 Whop.  ${minimalUrl} ${regularCta} #BTC #TrapScore #RiskOff #TrapDefence`
  };
  ...
  const variantE = {
    en: () =>
      `${stateHookEn} Trap Score = data vs gut. Free TG: «Score 12/100. Exit map—» Rest in Telegram. Regular Briefing: «Next trap—» Rest in Whop. Free: ${minimalUrl} ${regularCta} Watch:  #BTC #TrapDefence`,
    ja: () =>
      `${stateHookJa} Trap Scoreでdata vs gut。無料TG: «スコア12/100。出口マップ—» 続きはTGで。有料: «次の罠—» 続きはWhopで。無料: ${minimalUrl} ${regularCta} 視聴:  #BTC #TrapDefence`,
    es: () =>
      `¿Atrapado en el bucle de "solo mirar" con pérdida no realizada? Sin reglas = cada movimiento una suposición. Trap Score = datos vs instinto. TG gratis: «Score 12/100. Mapa—» Resto en Telegram. Regular Briefing: «Próxima trampa—» Resto en Whop. Gratis: ${minimalUrl} ${regularCta} Mira:  #BTC #TrapDefence`,
    "pt-br": () =>
      `Preso no loop de "só assistir" com perda não realizada? Sem regras = cada movimento um chute. Trap Score = dados vs gut. TG grátis: «Score 12/100. Mapa—» Resto no Telegram. Regular Briefing: «Próxima armadilha—» Resto no Whop. Grátis: ${minimalUrl} ${regularCta} Assista:  #BTC #TrapDefence`,
    ar: () =>
      `عالق في حلقة "فقط أشاهد" مع خسارة غير محققة؟ بدون قواعد = كل خطوة تخمين. Trap Score = بيانات vs غريزة. TG مجاني: «Score 12/100. خريطة—» الباقي في تيليجرام. Regular Briefing: «الفخ الجاي—» الباقي في Whop. مجاني: ${minimalUrl} ${regularCta} شوف:  #BTC #TrapDefence`,
    ko: () =>
      `미실현 손실로 "그냥 보기" 루프에 갇혀? 룰 없음 = 매번 추측. Trap Score = 데이터 vs 직감. 무료 TG: «스코어 12/100. 출구 맵—» 나머지는 TG. Regular Briefing: «다음 함정—» 나머지는 Whop. 무료: ${minimalUrl} ${regularCta} 시청:  #BTC #TrapDefence`
  };
...
/** 統合テンプレートのバリアント名（A〜H。H=テーマ渡し→セールスレター本文1回生成→下部CTA） */
const INTEGRATED_VARIANTS = ["A", "B", "C", "D", "E", "F", "H"];
