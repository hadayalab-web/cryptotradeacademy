/**
 * GPTコードレビュー - Vercelデプロイ前の健全性チェック
 * 
...
### 5. Grokエンハンスト (src/utils/grok-enhanced.ts) - 一部
\`\`\`typescript
${readCodeFile('src/utils/grok-enhanced.ts').substring(0, 2000)}
\`\`\`

### 6. GPTエンハンスト (src/utils/gpt-enhanced.ts) - 一部
\`\`\`typescript
${readCodeFile('src/utils/gpt-enhanced.ts').substring(0, 2000)}
\`\`\`

### 7. Geminiエンハンスト (src/utils/gemini-enhanced.ts) - 一部
\`\`\`typescript
${readCodeFile('src/utils/gemini-enhanced.ts').substring(0, 2000)}
\`\`\`
...
async function main() {
  try {
    ...
    // 結果をファイルに保存
    const outputPath = path.join(process.cwd(), 'GPT_CODE_REVIEW.md');
    
    const output = `# GPTコードレビュー - Vercelデプロイ前の健全性チェック
...
- src/utils/grok-enhanced.ts (一部)
- src/utils/gpt-enhanced.ts (一部)
- src/utils/gemini-enhanced.ts (一部)
