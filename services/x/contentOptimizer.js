  ...
  try {
    const completion = await grokClient.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'あなたはX（Twitter）アルゴリズムの専門家です。2026年の最新動向を分析し、エンゲージメント率とコンバージョン率を最大化するための戦略を提供します。',
        },
        {
          role: 'user',
          content: prompt,
...
async function optimizeContentAndFunnel(options = {}) {
  ...
  try {
    ...
    if (grokResult.status === 'rejected') {
      console.warn('[X Content Optimizer] ⚠️ Grok analysis failed:', grokResult.reason?.message || grokResult.reason);
    }
    if (geminiResult.status === 'rejected') {
      console.warn('[X Content Optimizer] ⚠️ Gemini analysis failed:', geminiResult.reason?.message || geminiResult.reason);
    }
