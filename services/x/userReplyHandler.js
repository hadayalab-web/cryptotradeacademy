...
async function getTweetReplies(tweetId, maxResults = 10) {
  try {
    ...
    console.error(
      `[User Reply Handler] Failed to get replies for tweet ${tweetId}:`,
      error.message
    );
    return [];
  }
...
async function handleUserReplies(mainTweetId, lang, reportData = null, maxReplies = 10) {
  ...
  if (!xStatus.postingEnabled || !xStatus.configured) {
    console.warn(
      "[User Reply Handler] X posting disabled or not configured, skipping user reply handling"
    );
    return [];
  }
  ...
  if (kv) {
    try {
      ...
      if (alreadyProcessed) {
        console.log(
          `[User Reply Handler] Already processed replies for tweet ${mainTweetId}, skipping`
        );
        return [];
      }
    } catch (error) {
  ...
  // P0 FIX: 処理開始直後に「処理済み」をマークし、重複実行（Cron二重起動・同時実行）で同じツイートを再処理しない
  if (kv) {
    try {
      const key = `x:user_replies:${mainTweetId}`;
      await kv.set(key, true, { ex: 86400 * 7 }); // 7日間保持
      console.log(
        `[User Reply Handler] Marked tweet ${mainTweetId} as processing (duplicate prevention)`
      );
    } catch (err) {
      console.warn("[User Reply Handler] Failed to set processing mark (non-fatal):", err.message);
    }
  }
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          console.log(
            `[User Reply Handler] ✅ Auto-reply sent to @${reply.author?.username || "unknown"} (${reply.id}): ${result.id}`
          );
          ...
          console.error(
            `[User Reply Handler] ❌ Failed to send auto-reply to ${reply.id}:`,
            error.message
          );
          results.push({
            replyId: reply.id,
  ...
  // 処理済みマークは上で「処理開始直後」に設定済み（重複防止）。results は setTimeout 内で非同期に増えるためここでは参照しない。

  console.log(
    `[User Reply Handler] Scheduled ${replies.length} user replies for tweet ${mainTweetId}`
  );
  return results;
}
